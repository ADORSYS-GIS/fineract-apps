...
import { Button, Card, Input, Select } from "@fineract-apps/ui";
import { ClipboardCheck, Clock, CheckCircle, XCircle, Search, Eye } from "lucide-react";
...
export function KycReviews() {
  ...
  const { data: submissions, isLoading: submissionsLoading } = useKycSubmissions({
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <StatCard
          ...
          value={statsLoading ? "..." : formatTime(stats?.avgReviewTimeMinutes || 0)}
          icon={Clock}
          color="purple"
        />
      </div>
      ...
      <Card className="p-4">
        <div className="flex flex-col md:flex-row gap-4">
          ...
          <div className="w-full md:w-48">
            ...
            >
              <Select.Trigger>
                <Select.Value placeholder={t("kycReviews.filterByStatus")} />
              </Select.Trigger>
              <Select.Content>
                <Select.Item value="all">{t("kycReviews.allStatuses")}</Select.Item>
                <Select.Item value="pending">{t("kycReviews.pending")}</Select.Item>
                <Select.Item value="approved">{t("kycReviews.approved")}</Select.Item>
                <Select.Item value="rejected">{t("kycReviews.rejected")}</Select.Item>
                <Select.Item value="info_requested">{t("kycReviews.infoRequested")}</Select.Item>
              </Select.Content>
            </Select>
          </div>
        </div>
      ...
      <Card>
        <div className="p-4 border-b">
          <h2 className="text-lg font-semibold">{t("kycReviews.allSubmissions")}</h2>
        </div>
