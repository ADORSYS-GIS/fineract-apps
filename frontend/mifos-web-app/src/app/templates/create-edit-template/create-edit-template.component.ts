...
export class CreateEditComponent implements OnInit {
	...
	constructor(
		...
		this.route.data.subscribe(
			(data: { templateData: any; mode: "create" | "edit" }) => {
				this.templateData = data.templateData;
				this.mode = data.mode;
				if (this.mode === "edit") {
					this.mappers = this.templateData.template.mappers.map(
						(mapper: any) => ({
	...
	createTemplateForm() {
		if (this.mode === "create") {
			...
			this.templateForm = this.formBuilder.group({
				...
				type: [
					this.templateData.types.find(
						(type: any) => type.name === this.templateData.template.type,
					).id,
					Validators.required,
				],
				name: [this.templateData.template.name, Validators.required],
				text: [this.templateData.template.text, Validators.required],
			});
		}
	}
		...
		this.templateForm.get("entity").valueChanges.subscribe((value: any) => {
			if (value === 0) {
				...
				this.mappers.splice(0, 1, {
					...
					mappersvalue: new UntypedFormControl(
						"loans/{{loanId}}?associations=all&tenantIdentifier=" +
							tenantIdentifier,
					),
				});
			}
	...
	submit() {
		...
		if (this.mode === "create") {
			...
			this.templateService
				.updateTemplate(template, this.templateData.template.id)
				.subscribe(() => {
