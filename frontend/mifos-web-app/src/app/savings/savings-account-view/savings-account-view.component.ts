...
export class SavingsAccountViewComponent implements OnInit {
	...
	constructor(
		...
		this.route.data.subscribe(
			(data: { savingsAccountData: any; savingsDatatables: any }) => {
	...
	doAction(name: string) {
		switch (name) {
			...
			case "Apply Annual Fees":
				...
				break;
			case "Withdrawal":
				this.router.navigate([`actions/Withdrawal`], {
	...
	private deleteSavingsAccount() {
		const deleteSavingsAccountDialogRef = this.dialog.open(
			...
			{
				data: {
					deleteContext: `savings account with id: ${this.savingsAccountData.id}`,
				},
			},
		);
		deleteSavingsAccountDialogRef.afterClosed().subscribe((response: any) => {
			if (response.delete) {
				this.savingsService
					.deleteSavingsAccount(this.savingsAccountData.id)
					.subscribe(() => {
	...
	private unblockSavingsAccount(action: string) {
		const unblockSavingsAccountDialogRef = this.dialog.open(
			ConfirmationDialogComponent,
			{
				data: {
					heading: this.translateService.instant(
						"labels.heading.Savings Account",
					),
					dialogContext:
						this.translateService.instant(
							"labels.dialogContext.Are you sure you want",
						) +
						action +
