...
@Component({
	...
	imports: [...STANDALONE_SHARED_IMPORTS, EntityNotesTabComponent],
})
export class NotesTabComponent {
	...
	addNote(noteContent: any) {
		this.savingsService
			.createSavingsNote(this.entityId, noteContent)
	...
	editNote(noteId: string, noteContent: any, index: number) {
		this.savingsService
			.editSavingsNote(this.entityId, noteId, noteContent)
			.subscribe(() => {
	...
	deleteNote(noteId: string, index: number) {
		this.savingsService
			.deleteSavingsNote(this.entityId, noteId)
			.subscribe(() => {
