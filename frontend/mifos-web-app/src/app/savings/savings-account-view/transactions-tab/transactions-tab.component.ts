...
export class TransactionsTabComponent implements OnInit {
	...
	setTransactions(): void {
		...
		this.accountWithTransactions =
			this.transactionsData && this.transactionsData.length > 0;
	...
	isAccrual(transactionType: SavingsAccountTransactionType): boolean {
		return (
			...
			transactionType.code === "savingsAccountTransactionType.accrual"
		);
	}
	...
	showTransactions(transactionsData: SavingsAccountTransaction) {
		if (transactionsData.transfer) {
			...
			this.router.navigate([transactionsData.id, "general"], {
				relativeTo: this.route,
			});
		}
	}
	...
	filterTransactions(hideReversed: boolean, hideAccrual: boolean): void {
		...
		if (hideAccrual || hideReversed) {
			transactions = this.transactionsData.filter(
				(t: SavingsAccountTransaction) => {
					return (
	...
	private reload() {
		...
		this.router
			.navigateByUrl(`/clients/${clientId}/savings-accounts`, {
