...
@Component({
	...
	imports: [...STANDALONE_SHARED_IMPORTS, MatCardTitle, InputAmountComponent],
})
export class ManageSavingsAccountComponent implements OnInit {
	...
	constructor(
		...
		this.transactionCommand = this.route.snapshot.params["name"]
			.toLowerCase()
			.replaceAll(" ", "");
	...
	submit() {
		...
		if (this.transactionType.holdamount) {
			...
			const prevTransactionDate: Date =
				this.manageSavingsAccountForm.value.transactionDate;
			if (manageSavingsAccountFormData.transactionDate instanceof Date) {
				manageSavingsAccountFormData.transactionDate =
