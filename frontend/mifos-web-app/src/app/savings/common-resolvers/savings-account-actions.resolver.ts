...
export class SavingsAccountActionsResolver {
	...
	resolve(route: ActivatedRouteSnapshot): Observable<any> {
		...
		switch (actionName) {
			case "Assign Staff":
				return this.savingsService.getSavingsAccountAndTemplate(
					savingAccountId,
					true,
				);
			case "Add Charge":
				return this.savingsService.getSavingsChargeTemplateResource(
					savingAccountId,
				);
			case "Withdrawal":
			case "Deposit":
			case "Hold Amount":
				return this.savingsService.getSavingsTransactionTemplateResource(
					savingAccountId,
				);
			case "Close":
				return forkJoin([
					this.savingsService.getSavingsTransactionTemplateResource(
						savingAccountId,
					),
