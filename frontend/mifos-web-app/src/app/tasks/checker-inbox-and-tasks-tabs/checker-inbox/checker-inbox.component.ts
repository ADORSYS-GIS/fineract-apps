...
export class CheckerInboxComponent implements OnInit {
	...
	constructor(
		...
		this.route.data.subscribe(
			(data: { makerCheckerResource: any; makerCheckerTemplate: any }) => {
	...
	approveChecker() {
		const approveCheckerDialogRef = this.dialog.open(
			ConfirmationDialogComponent,
			{
				data: {
					heading: this.translateService.instant(
						"labels.heading.Approve Checker",
					),
	...
	rejectChecker() {
		const rejectCheckerDialogRef = this.dialog.open(
			ConfirmationDialogComponent,
			{
				data: {
					heading: this.translateService.instant(
						"labels.heading.Reject Checker",
					),
					dialogContext: this.translateService.instant(
	...
	deleteChecker() {
		const deleteCheckerDialogRef = this.dialog.open(
			ConfirmationDialogComponent,
			{
				data: {
					heading: this.translateService.instant(
						"labels.heading.Delete Checker",
					),
					dialogContext: this.translateService.instant(
	...
	bulkCheckerApproveorReject(action: any) {
		...
		listSelectedAccounts.forEach((element: any) => {
			this.tasksService
				.executeMakerCheckerAction(element.id, action)
				.subscribe((response: any) => {
					approvedAccounts++;
