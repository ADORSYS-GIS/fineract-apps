...
export class ClientApprovalComponent {
	...
	constructor(
		...
		this.route.data.subscribe((data: { groupedClientData: any }) => {
			this.groupedClients = _.groupBy(
				data.groupedClientData.pageItems,
				"officeName",
			);
	...
	approveClients() {
		...
		const clientApprovalDialogRef = this.dialog.open(FormDialogComponent, {
			data,
		});
	...
	bulkClientApproval(submittedData: any) {
		...
		const activationDate = this.dateUtils.formatDate(
			submittedData.value.actDate,
			dateFormat,
		);
		...
		this.tasksService
			.submitBatchData(this.batchRequests)
