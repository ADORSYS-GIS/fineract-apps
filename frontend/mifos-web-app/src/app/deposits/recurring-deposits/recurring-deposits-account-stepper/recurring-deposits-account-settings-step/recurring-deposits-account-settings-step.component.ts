...
export class RecurringDepositsAccountSettingsStepComponent
	implements OnInit, OnChanges
{
		...
		if (recurringDepositsAccount) {
			...
			this.recurringDepositAccountSettingsForm.patchValue({
				...
				lockinPeriodFrequencyType:
					recurringDepositsAccount.lockinPeriodFrequencyType
				...
				inMultiplesOfDepositTerm:
					recurringDepositsAccount.inMultiplesOfDepositTerm,
				inMultiplesOfDepositTermTypeId:
					recurringDepositsAccount.inMultiplesOfDepositTermType
				...
				preClosurePenalApplicable:
					recurringDepositsAccount.preClosurePenalApplicable,
				preClosurePenalInterest:
					recurringDepositsAccount.preClosurePenalInterest,
				preClosurePenalInterestOnTypeId:
					recurringDepositsAccount.preClosurePenalInterestOnType
						? recurringDepositsAccount.preClosurePenalInterestOnType.id
						: "",
				minBalanceForInterestCalculation:
					recurringDepositsAccount.minBalanceForInterestCalculation,
					...
					.valueChanges.subscribe((value: boolean) => {
						if (value) {
							...
							this.recurringDepositAccountSettingsForm
								.get("taxGroupId")
								.patchValue(
									recurringDepositsAccount.taxGroup &&
										recurringDepositsAccount.taxGroup.name,
								);
						} else {
							this.recurringDepositAccountSettingsForm.removeControl(
								"taxGroupId",
							);
						}
					});
		...
		if (this.recurringDepositsAccountTemplate) {
			this.recurringDepositAccountSettingsForm.patchValue({
				lockinPeriodFrequency:
					this.recurringDepositsAccountTemplate.lockinPeriodFrequency,
				lockinPeriodFrequencyType:
	...
	setOptions(recurringDepositsAccount: any) {
		this.lockinPeriodFrequencyTypeData =
			recurringDepositsAccount.lockinPeriodFrequencyTypeOptions;
		this.periodFrequencyTypeData =
			recurringDepositsAccount.periodFrequencyTypeOptions;
