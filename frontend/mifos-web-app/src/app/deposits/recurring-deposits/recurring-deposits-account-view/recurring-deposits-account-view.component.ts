...
export class RecurringDepositsAccountViewComponent implements OnInit {
		...
		this.route.data.subscribe(
			(data: { recurringDepositsAccountData: any; savingsDatatables: any }) => {
				...
				this.isprematureAllowed =
					data.recurringDepositsAccountData.maturityDate != null;
	...
	doAction(name: string) {
		switch (name) {
			...
			case "Modify Application":
				this.router.navigate(["edit-recurring-deposit-account"], {
	...
	private deleteRecurringDepositsAccount() {
		const deleteRecurringDepositsAccountDialogRef = this.dialog.open(
		...
		deleteRecurringDepositsAccountDialogRef
			.afterClosed()
			.subscribe((response: any) => {
				if (response.delete) {
					this.recurringDepositsService
						.deleteRecurringDepositsAccount(
			...
			RecurringDepositConfirmationDialogComponent,
			{
				data: {
					heading: this.translateService.instant(
						"labels.heading.Calculate Interest",
					),
	...
	private postInterest() {
		const postInterestAccountDialogRef = this.dialog.open(
			RecurringDepositConfirmationDialogComponent,
			{
				data: {
					heading: this.translateService.instant(
						"labels.heading.Post Interest",
					),
					dialogContext:
						this.translateService.instant(
							"labels.text.Are you sure you want to post interest",
						) + " ?",
				},
			},
		);
	...
	private enableWithHoldTax() {
		const deleteSavingsAccountDialogRef = this.dialog.open(
			RecurringDepositConfirmationDialogComponent,
			{
				data: {
					heading: this.translateService.instant(
						"labels.heading.Enable Withhold Tax",
					),
					dialogContext: this.translateService.instant(
	...
	private disableWithHoldTax() {
		const disableWithHoldTaxDialogRef = this.dialog.open(
			RecurringDepositConfirmationDialogComponent,
			{
				data: {
					heading: this.translateService.instant(
						"labels.heading.Disable Withhold Tax",
					),
					dialogContext: this.translateService.instant(
