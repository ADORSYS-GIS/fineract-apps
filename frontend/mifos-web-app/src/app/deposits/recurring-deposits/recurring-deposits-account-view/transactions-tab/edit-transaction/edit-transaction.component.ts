...
@Component({
	...
	imports: [...STANDALONE_SHARED_IMPORTS, InputAmountComponent, MatSlideToggle],
})
export class EditTransactionComponent implements OnInit {
	...
	constructor(
		...
		this.route.data.subscribe(
			(data: { recurringDepositsAccountTransactionTemplate: any }) => {
				this.transactionTemplateData =
					data.recurringDepositsAccountTransactionTemplate;
				if (this.transactionTemplateData.currency) {
					this.currency = this.transactionTemplateData.currency;
				}
				this.paymentTypeOptions =
					this.transactionTemplateData.paymentTypeOptions;
			},
		);
	...
	ngOnInit() {
		...
		this.editTransactionForm.patchValue({
			transactionDate:
				this.transactionTemplateData.date &&
				new Date(this.transactionTemplateData.date),
	...
	addPaymentDetails() {
		...
		if (this.addPaymentDetailsFlag) {
			this.editTransactionForm.addControl(
				"accountNumber",
	...
	submit() {
		...
		const prevTransactionDate: Date =
			this.editTransactionForm.value.transactionDate;
