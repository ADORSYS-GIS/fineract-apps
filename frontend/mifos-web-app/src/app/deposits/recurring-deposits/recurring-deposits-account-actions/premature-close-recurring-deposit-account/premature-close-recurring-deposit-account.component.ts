...
export class PrematureCloseRecurringDepositAccountComponent implements OnInit {
	...
	submit() {
		...
		if (
			prematureCloseRecurringDepositsAccountFormData.closedOnDate instanceof
			Date
		) {
			prematureCloseRecurringDepositsAccountFormData.closedOnDate =
				this.dateUtils.formatDate(prevClosedOnDate, dateFormat);
		}
		const data = {
