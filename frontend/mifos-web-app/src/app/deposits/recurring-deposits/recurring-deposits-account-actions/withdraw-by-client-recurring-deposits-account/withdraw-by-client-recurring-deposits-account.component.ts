	...
	implements OnInit
{
	...
	submit() {
		...
		if (
			withdrawRecurringDepositsAccountFormData.withdrawnOnDate instanceof Date
		) {
			withdrawRecurringDepositsAccountFormData.withdrawnOnDate =
				this.dateUtils.formatDate(prevWithdrawnOnDate, dateFormat);
		}
		const data = {
