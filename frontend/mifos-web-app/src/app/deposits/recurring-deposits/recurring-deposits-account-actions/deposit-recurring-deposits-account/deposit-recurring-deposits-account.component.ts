...
export class DepositRecurringDepositsAccountComponent implements OnInit {
	...
	constructor(
		...
		this.route.data.subscribe(
			(data: { recurringDepositsAccountActionData: any }) => {
				this.transactionAmount = data.recurringDepositsAccountActionData.amount;
				this.paymentTypes =
					data.recurringDepositsAccountActionData.paymentTypeOptions;
				if (
	...
	addPaymentDetails() {
		...
		if (this.showPaymentDetails) {
			this.depositRecurringDepositForm.addControl(
				"accountNumber",
	 ...
	 */
	submit() {
		const depositRecurringDepositFormData =
			this.depositRecurringDepositForm.value;
		const dateFormat = this.settingsService.dateFormat;
		const locale = this.settingsService.language.code;
		const prevTransactionDate =
			this.depositRecurringDepositForm.value.transactionDate;
		if (depositRecurringDepositFormData.transactionDate instanceof Date) {
			depositRecurringDepositFormData.transactionDate =
				this.dateUtils.formatDate(prevTransactionDate, dateFormat);
		}
		const data = {
