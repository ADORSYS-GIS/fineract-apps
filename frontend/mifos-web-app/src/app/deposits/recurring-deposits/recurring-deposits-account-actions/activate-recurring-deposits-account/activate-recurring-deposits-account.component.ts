...
export class ActivateRecurringDepositsAccountComponent implements OnInit {
	...
	submit() {
		...
		if (
			activateRecurringDepositsAccountFormData.activatedOnDate instanceof Date
		) {
			activateRecurringDepositsAccountFormData.activatedOnDate =
				this.dateUtils.formatDate(prevActivatedOnDate, dateFormat);
		}
		const data = {
