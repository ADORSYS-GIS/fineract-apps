...
@Component({
	...
	imports: [...STANDALONE_SHARED_IMPORTS, CdkTextareaAutosize],
})
export class ApproveRecurringDepositsAccountComponent implements OnInit {
	...
	constructor(
		...
		this.accountId =
			this.route.parent.snapshot.params["recurringDepositAccountId"];
	}
	 ...
	 */
	submit() {
		const approveRecurringDepositsAccountFormData =
			this.approveRecurringDepositsAccountForm.value;
		const locale = this.settingsService.language.code;
		const dateFormat = this.settingsService.dateFormat;
		const prevApprovedOnDate: Date =
			this.approveRecurringDepositsAccountForm.value.approvedOnDate;
		if (
			approveRecurringDepositsAccountFormData.approvedOnDate instanceof Date
		) {
			approveRecurringDepositsAccountFormData.approvedOnDate =
				this.dateUtils.formatDate(prevApprovedOnDate, dateFormat);
		}
		const data = {
