...
export class RejectRecurringDepositsAccountComponent implements OnInit {
	 ...
	 */
	submit() {
		const rejectRecurringDepositsAccountFormData =
			this.rejectRecurringDepositsAccountForm.value;
		const locale = this.settingsService.language.code;
		const dateFormat = this.settingsService.dateFormat;
		const prevRejectedOnDate: Date =
			this.rejectRecurringDepositsAccountForm.value.rejectedOnDate;
		if (rejectRecurringDepositsAccountFormData.rejectedOnDate instanceof Date) {
			rejectRecurringDepositsAccountFormData.rejectedOnDate =
				this.dateUtils.formatDate(prevRejectedOnDate, dateFormat);
		}
		const data = {
