...
export class EditFixedDepositAccountComponent {
	...
	constructor(
		...
		this.route.data.subscribe(
			(data: { fixedDepositsAccountAndTemplate: any }) => {
				this.fixedDepositsAccountAndTemplate =
					data.fixedDepositsAccountAndTemplate;
			},
		);
	}
	...
	submit() {
		...
		const fixedDepositAccount = {
			...
			charges: this.fixedDepositAccount.charges.map((charge: any) => ({
				...
				feeOnMonthDay:
					...
					this.dateUtils.formatDate(
						[2000].concat(charge.feeOnMonthDay),
						monthDayFormat,
					),
			...
			charts: [
				{
					chartSlabs:
						this.fixedDepositsAccountProductTemplate.accountChart.chartSlabs,
				},
			],
			dateFormat,
			monthDayFormat,
		...
		this.fixedDepositsService
			.updateFixedDepositAccount(
				this.fixedDepositsAccountAndTemplate.id,
				fixedDepositAccount,
			)
