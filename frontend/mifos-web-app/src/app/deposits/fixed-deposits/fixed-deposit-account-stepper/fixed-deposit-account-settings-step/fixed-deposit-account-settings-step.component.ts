...
export class FixedDepositAccountSettingsStepComponent
	implements OnInit, OnChanges
{
	...
	ngOnChanges() {
		if (this.fixedDepositsAccountProductTemplate) {
			this.fixedDepositAccountSettingsForm.patchValue({
				...
				minDepositTermTypeId: this.fixedDepositsAccountProductTemplate
					.minDepositTermType
					? this.fixedDepositsAccountProductTemplate.minDepositTermType.id
					: "",
				inMultiplesOfDepositTerm:
					this.fixedDepositsAccountProductTemplate.inMultiplesOfDepositTerm,
				inMultiplesOfDepositTermTypeId: this.fixedDepositsAccountProductTemplate
					...
					? this.fixedDepositsAccountProductTemplate
							.inMultiplesOfDepositTermType.id
					: "",
				maxDepositTerm: this.fixedDepositsAccountProductTemplate.maxDepositTerm,
				maxDepositTermTypeId: this.fixedDepositsAccountProductTemplate
					.maxDepositTermType
				...
				preClosurePenalInterest:
					this.fixedDepositsAccountProductTemplate.preClosurePenalInterest,
				preClosurePenalInterestOnTypeId: this
					.fixedDepositsAccountProductTemplate.preClosurePenalInterestOnType
					? this.fixedDepositsAccountProductTemplate
							.preClosurePenalInterestOnType.id
					: "",
				maturityInstructionId:
					this.fixedDepositsAccountProductTemplate.maturityInstructionId,
			});
		...
		if (this.fixedDepositsAccountTemplate) {
			this.fixedDepositAccountSettingsForm.patchValue({
				lockinPeriodFrequency:
					this.fixedDepositsAccountTemplate.lockinPeriodFrequency,
				...
				transferInterestToSavings:
					this.fixedDepositsAccountTemplate.transferInterestToSavings,
			});
		}
	}
		...
		this.fixedDepositAccountSettingsForm
			...
			.valueChanges.subscribe((value: number) => {
				if (value > 100) {
					...
					this.fixedDepositAccountSettingsForm.removeControl(
						"transferToSavingsId",
					);
				}
			});
	...
	setOptions() {
		...
		this.savingsAccountsData =
			this.fixedDepositsAccountProductTemplate.savingsAccounts;
	...
	get fixedDepositAccountSettings() {
		const fixedDepositAccountSettings =
			this.fixedDepositAccountSettingsForm.getRawValue();
