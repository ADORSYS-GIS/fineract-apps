...
export class CloseFixedDepositsAccountComponent implements OnInit {
	...
	constructor(
		...
		this.route.data.subscribe(
			(data: { fixedDepositsAccountActionData: any }) => {
				this.savingsAccountsData =
					data.fixedDepositsAccountActionData.savingsAccounts;
				this.onAccountClosureOptions =
					data.fixedDepositsAccountActionData.onAccountClosureOptions;
				this.maturityAmount =
					data.fixedDepositsAccountActionData.maturityAmount;
			},
		);
	 ...
	 */
	submit() {
		const closeOnMaturityAccountFormData =
			this.closeOnMaturityAccountForm.value;
		const locale = this.settingsService.language.code;
		const dateFormat = this.settingsService.dateFormat;
		const prevClosedDate: Date =
			this.closeOnMaturityAccountForm.value.closedOnDate;
