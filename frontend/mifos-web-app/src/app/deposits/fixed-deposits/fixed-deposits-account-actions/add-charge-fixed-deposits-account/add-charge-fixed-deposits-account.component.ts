...
export class AddChargeFixedDepositsAccountComponent implements OnInit {
	...
	constructor(
		...
		this.route.data.subscribe(
			(data: { fixedDepositsAccountActionData: any }) => {
				this.savingsChargeOptions =
					data.fixedDepositsAccountActionData.chargeOptions;
			},
		);
		this.fixedDepositAccountId =
			this.route.parent.snapshot.params["fixedDepositAccountId"];
	}
	...
	buildDependencies() {
		this.fixedDepositsChargeForm.controls.chargeId.valueChanges.subscribe(
			(chargeId) => {
				this.savingsService
		...
		if (this.chargeDetails.dueDateNotRequired !== true) {
			if (this.chargeDetails.chargeTimeTypeAnnualOrMonth === true) {
				...
				if (savingsCharge.feeOnMonthDay) {
					...
					savingsCharge.feeOnMonthDay = this.dateUtils.formatDate(
						prevDate,
						monthDayFormat,
					);
				}
				...
				if (savingsCharge.dueDate) {
					...
					savingsCharge.dueDate = this.dateUtils.formatDate(
						prevDate,
						dateFormat,
					);
				}
