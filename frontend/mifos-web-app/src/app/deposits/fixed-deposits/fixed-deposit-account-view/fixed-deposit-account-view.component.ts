...
export class FixedDepositAccountViewComponent implements OnInit {
	...
	constructor(
		...
		this.route.data.subscribe(
			(data: { fixedDepositsAccountData: any; savingsDatatables: any }) => {
	...
	private deleteFixedDepositsAccount() {
		const deleteFixedDepositsAccountDialogRef = this.dialog.open(
			DeleteDialogComponent,
			{
				data: {
					deleteContext: `fixed deposit account with id: ${this.fixedDepositsAccountData.id}`,
				},
			},
		);
