...
export class ChargesTabComponent implements OnInit {
	...
	@ViewChild("chargesTable", { static: true })
	chargesTableRef: MatTable<Element>;
	...
	constructor(
		...
		this.route.parent.data.subscribe(
			(data: { fixedDepositsAccountData: any }) => {
	...
	toggleCharges() {
		...
		if (!this.showInactiveCharges) {
			const activeCharges = this.chargesData.filter(
				(charge) => charge.isActive,
			);
			this.dataSource.data = activeCharges;
		} else {
			const inActiveCharges = this.chargesData.filter(
				(charge) => !charge.isActive,
			);
	...
	payCharge(chargeId: any) {
		...
		payChargeDialogRef.afterClosed().subscribe((response: any) => {
			if (response.data) {
				...
				const dataObject = {
					...response.data.value,
					dueDate: this.dateUtils.formatDate(
						response.data.value.dueDate,
						dateFormat,
					),
					dateFormat,
					locale,
	...
	inactivateCharge(chargeId: any) {
		const inactivateChargeDialogRef = this.dialog.open(
	...
	private reload() {
		...
		this.router
			.navigateByUrl(`/clients/${clientId}/fixed-deposits-accounts`, {
