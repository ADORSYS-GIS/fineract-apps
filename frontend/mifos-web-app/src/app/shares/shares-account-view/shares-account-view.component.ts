...
export class SharesAccountViewComponent implements OnInit {
	...
	private deleteSharesAccount() {
		const deleteSharesAccountDialogRef = this.dialog.open(
			DeleteDialogComponent,
			{
				data: {
					deleteContext: `shares account with id: ${this.sharesAccountData.id}`,
				},
			},
		);
		deleteSharesAccountDialogRef.afterClosed().subscribe((response: any) => {
			if (response.delete) {
				this.sharesService
					.deleteSharesAccount(this.sharesAccountData.id)
					.subscribe(() => {
