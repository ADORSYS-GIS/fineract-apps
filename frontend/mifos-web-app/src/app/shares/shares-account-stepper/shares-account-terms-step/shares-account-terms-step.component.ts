...
export class SharesAccountTermsStepComponent implements OnChanges, OnInit {
	...
	ngOnChanges() {
		if (this.sharesAccountProductTemplate) {
			...
			this.sharesAccountTermsForm.patchValue({
				...
				currencyMultiple:
					this.sharesAccountProductTemplate.currency.inMultiplesOf,
			...
			if (this.sharesAccountTemplate) {
				if (
					!this.isSavingsPatched &&
					this.sharesAccountTemplate.savingsAccountId
				) {
	...
	ngOnInit() {
		...
		if (this.sharesAccountTemplate) {
			this.sharesAccountTermsForm.patchValue({
				requestedShares:
					this.sharesAccountTemplate.summary.totalPendingForApprovalShares,
	...
	setOptions() {
		...
		this.savingsAccountsData =
			this.sharesAccountProductTemplate.clientSavingsAccounts;
	}
