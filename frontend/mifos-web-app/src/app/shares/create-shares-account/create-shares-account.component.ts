...
export class CreateSharesAccountComponent {
	...
	get sharesAccountFormValid() {
		return (
			this.sharesAccountDetailsForm.valid && this.sharesAccountTermsForm.valid
		);
	}
	...
	submit() {
		...
		const sharesAccount = {
			...
			applicationDate: this.dateUtils.formatDate(
				this.sharesAccount.applicationDate,
				dateFormat,
			),
			submittedDate: this.dateUtils.formatDate(
				this.sharesAccount.submittedDate,
				dateFormat,
			),
		...
		this.sharesService
			.createSharesAccount(sharesAccount)
