...
export class ManageSchedulerJobsComponent implements OnInit, AfterViewInit {
	...
	@ViewChild("schedulerStatus", { static: true })
	schedulerStatus: ElementRef<any>;
	/* Template for popover on scheduler status */
	@ViewChild("templateSchedulerStatus", { static: true })
	templateSchedulerStatus: TemplateRef<any>;
	...
	setJobs() {
		this.systemService.getJobs().subscribe((jobData: any[]) => {
			const sortedData = jobData.sort(
				(a, b) => b.active - a.active || this.sortByName(a, b),
			);
	...
	ngAfterViewInit() {
		...
		if (this.configurationWizardService.showSchedulerJobsList === true) {
			setTimeout(() => {
				this.showPopover(
					this.templateJobsTable,
					this.jobsTable.nativeElement,
					"top",
					true,
				);
			});
