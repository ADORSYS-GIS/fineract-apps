...
export class WorkflowJobsComponent implements OnInit {
	...
	ngOnInit(): void {
		this.systemService
			...
			.toPromise()
			.then((jobNames) => {
				this.jobNameOptions = jobNames.businessJobs.sort(function (
					a: any,
					b: any,
				) {
	...
	getWorkflowJobSteps(jobName: string) {
		this.systemService.getWorkflowJobSteps(jobName).subscribe((data: any) => {
			...
			this.jobStepsData = data.businessSteps.sort(function (
				a: JobStep,
				b: JobStep,
			) {
	...
	dropTable(event: CdkDragDrop<JobStep[]>) {
		const prevIndex = this.jobStepsData.findIndex(
			(d: JobStep) => d === event.item.data,
		);
	...
	addJobStep() {
		if (this.jobStepName != null) {
			...
			this.systemService
				...
				.toPromise()
				.then((jobData) => {
					this.jobAvailableStepsData = jobData.availableBusinessSteps.sort(
					...
					if (this.jobAvailableStepsData.length > 0) {
						this.jobAvailableStepsData = this.jobAvailableStepsData.filter(
							(item: any) => {
					...
					if (this.jobAvailableStepsData.length > 0) {
						for (
							let index = 0;
							index < this.jobAvailableStepsData.length;
							index++
						) {
							this.jobAvailableStepsData[index].stepDescription =
						...
						const data = {
							title: this.translateService.instant(
								"labels.text.Add Job Step to Workflow",
							),
							layout: { addButtonText: "Add" },
							formfields: frmFields,
						};
						const stepDialogRef = this.dialog.open(FormDialogComponent, {
							data,
						});
