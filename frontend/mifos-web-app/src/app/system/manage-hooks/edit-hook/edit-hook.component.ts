...
export class EditHookComponent implements OnInit {
			...
			smsProvider: [
				{
					value:
						this.hookData.name === "SMS Bridge"
							? this.hookData.config[2].fieldValue
							: "",
					disabled: this.hookData.name !== "SMS Bridge",
				},
				Validators.required,
			...
			smsProviderAccountId: [
				{
					value:
						this.hookData.name === "SMS Bridge"
							? this.hookData.config[3].fieldValue
							: "",
					disabled: this.hookData.name !== "SMS Bridge",
				},
				Validators.required,
			...
			smsProviderToken: [
				{
					value:
						this.hookData.name === "SMS Bridge"
							? this.hookData.config[4].fieldValue
							: "",
					disabled: this.hookData.name !== "SMS Bridge",
				},
				Validators.required,
			...
			data: {
				deleteContext:
					this.translateService.instant(
						"labels.inputs.event with entity name of",
					) +
					" " +
	...
	submit() {
		const hook: {
			...
			config: {
				...
				"SMS Provider": this.hookForm.get("smsProvider").enabled
					...
					: undefined,
				"SMS Provider Account Id": this.hookForm.get("smsProviderAccountId")
					.enabled
		...
		this.systemService
			.updateHook(this.hookData.id, hook)
