...
export class SystemService {
	public static CONFIG_ASSET_EXTERNALIZATION =
	...
	runSelectedJob(jobId: string): Promise<any> {
		return this.http
			.post(`/jobs/${jobId}?command=executeJob`, this.emptyPayload, {
				observe: "response",
			})
			.toPromise();
	}
	...
	runSelectedJobWithParameters(
		...
		return this.http
			.post(`/jobs/${jobId}?command=executeJob`, jobParameters, {
				observe: "response",
			})
			.toPromise();
	}
	...
	updateExternalConfiguration(
		...
		return this.http.put(
			`/externalservice/${externalConfigurationName}`,
	 ...
	 */
	getEntitytoEntityData(
		mappingId: number,
		fromId: number,
		toId: number,
	): Observable<any> {
		return this.http.get(
			`/entitytoentitymapping/${mappingId}/${fromId}/${toId}`,
		);
	}
	...
	getEntityDatatable(entityId: string, datatableName: string) {
		...
		return this.http.get(`/datatables/${datatableName}/${entityId}`, {
			params: httpParams,
		});
	}
	...
	addEntityDatatableEntry(entityId: string, datatableName: string, data: any) {
		const httpParams = new HttpParams().set("genericResultSet", "true");
		return this.http.post(`/datatables/${datatableName}/${entityId}`, data, {
			params: httpParams,
		});
	}
	...
	editEntityDatatableEntry(entityId: string, datatableName: string, data: any) {
		const httpParams = new HttpParams().set("genericResultSet", "true");
		return this.http.put(`/datatables/${datatableName}/${entityId}`, data, {
			params: httpParams,
		});
	}
	...
	deleteDatatableContent(entityId: string, datatableName: string) {
		const httpParams = new HttpParams().set("genericResultSet", "true");
		return this.http.delete(`/datatables/${datatableName}/${entityId}`, {
			params: httpParams,
		});
	}
	...
	deleteDatatableEntry(entityId: string, rowId: string, datatableName: string) {
		const httpParams = new HttpParams().set("genericResultSet", "true");
		return this.http.delete(
			`/datatables/${datatableName}/${entityId}/${rowId}`,
			{ params: httpParams },
		);
	}
