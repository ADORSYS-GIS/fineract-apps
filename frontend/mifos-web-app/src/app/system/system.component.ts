...
export class SystemComponent implements AfterViewInit {
	...
	@ViewChild("templateRolesandPermission")
	templateRolesandPermission: TemplateRef<any>;
	...
	@ViewChild("templateMakerCheckerTable")
	templateMakerCheckerTable: TemplateRef<any>;
	...
	ngAfterViewInit() {
		if (this.configurationWizardService.showDatatables === true) {
			setTimeout(() => {
				this.showPopover(
					this.templateDatatables,
					this.datatables.nativeElement,
					"bottom",
					true,
				);
			});
