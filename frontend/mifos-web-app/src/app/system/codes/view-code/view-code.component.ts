...
export class ViewCodeComponent implements OnInit {
	...
	createCodeValuesRow(codeValue?: any): UntypedFormGroup {
		return this.formBuilder.group({
			...
			description: [
				{ value: codeValue ? codeValue.description : "", disabled: true },
			],
			position: [
			...
			isActive: [
				{ value: codeValue ? codeValue.active : false, disabled: true },
			],
		});
	}
	...
	deleteCodeValue(index: number) {
		...
		this.systemService
			.deleteCodeValue(this.codeData.id, codeValueId)
	...
	delete() {
		const deleteCodeDialogRef = this.dialog.open(DeleteDialogComponent, {
			data: {
				deleteContext:
					this.translateService.instant("labels.inputs.Code") +
					" " +
					this.codeData.name,
			},
		});
	...
	disableRow(index: number) {
		this.codeValues
			.at(index)
			.get("name")
			.setValue(this.codeValuesData[index].name);
		this.codeValues
			.at(index)
			.get("description")
			.setValue(this.codeValuesData[index].description);
		this.codeValues
			.at(index)
			.get("position")
			.setValue(this.codeValuesData[index].position);
		this.codeValues
			.at(index)
			.get("isActive")
			.setValue(this.codeValuesData[index].isActive);
		this.codeValues.at(index).disable();
		this.codeValueRowStatus[index] = "disabled";
		this.codeValues.at(index).markAsPristine();
	}
