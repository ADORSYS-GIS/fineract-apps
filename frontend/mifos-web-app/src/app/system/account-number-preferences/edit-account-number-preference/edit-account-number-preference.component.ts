...
export class EditAccountNumberPreferenceComponent implements OnInit {
	...
	constructor(
		...
		this.route.data.subscribe(
			(data: {
				accountNumberPreference: any;
				accountNumberPreferencesTemplate: any;
			}) => {
				this.accountNumberPreferenceData = data.accountNumberPreference;
				this.accountNumberPreferencesTemplateData =
					data.accountNumberPreferencesTemplate;
			},
		);
	}
	...
	createAccountNumberPreferenceForm() {
		this.accountNumberPreferenceForm = this.formBuilder.group({
			accountType: [
				{
					value: this.accountNumberPreferenceData.accountType.id,
					disabled: true,
				},
				Validators.required,
			],
