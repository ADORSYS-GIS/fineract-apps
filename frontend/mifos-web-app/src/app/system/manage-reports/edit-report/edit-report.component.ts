...
export class EditReportComponent implements OnInit {
	...
	constructor(
		...
		this.route.data.subscribe((data: { report: any; reportTemplate: any }) => {
			...
			this.reportParametersData = data.report.reportParameters
				? data.report.reportParameters
				: [];
	...
	createReportForm() {
		this.reportForm = this.formBuilder.group({
			reportName: [
				{
					value: this.reportData.reportName,
			...
			reportCategory: [
				{
					value: this.reportData.reportCategory
						? this.reportData.reportCategory
						: "",
					disabled: this.reportData.coreReport,
				},
			],
			...
			reportType: [
				{
					value: this.reportData.reportType,
					disabled: this.reportData.coreReport,
				},
				Validators.required,
			],
			reportSubType: [
				{
					value: this.reportData.reportSubType
						? this.reportData.reportSubType
						: "",
					disabled:
						this.reportData.reportType !== "Chart" ||
						this.reportData.coreReport,
				},
			],
			useReport: [
				this.reportData.useReport ? this.reportData.useReport : false,
			],
			reportSql: [
				{
	...
	addReportParameter() {
		...
		const addReportParameterDialogRef = this.dialog.open(
			...
			if (response !== "") {
				this.reportParametersData.push({
					...
					parameterName: this.reportTemplateData.allowedParameters.find(
						(allowedParameter: any) =>
							allowedParameter.id === response.parameterName,
					).parameterName,
	...
	editReportParameter(reportParameter: any) {
		...
		this.dataForDialog.reportParameterName =
			reportParameter.reportParameterName;
		const editReportParameterDialogRef = this.dialog.open(
			ReportParameterDialogComponent,
			{
				data: this.dataForDialog,
			},
		...
		editReportParameterDialogRef.afterClosed().subscribe((response: any) => {
			if (response !== "") {
				this.reportParametersData[
					this.reportParametersData.indexOf(reportParameter)
				] = {
	...
	deleteReportParameter(reportParameter: any) {
		const deleteReportDialogRef = this.dialog.open(DeleteDialogComponent, {
			data: {
				deleteContext: `report parameter ${reportParameter.parameterName}`,
			},
		});
		deleteReportDialogRef.afterClosed().subscribe((response: any) => {
			if (response.delete) {
				this.reportParametersData.splice(
					this.reportParametersData.indexOf(reportParameter),
					1,
				);
				this.dataSource.connect().next(this.reportParametersData);
	...
	submit() {
		if (this.reportData.coreReport) {
			...
			this.reportForm.value.reportParameters.map(function (
				reportParameter: any,
			) {
		...
		this.systemService
			.updateReport(this.reportData.id, this.reportForm.value)
			.subscribe(() => {
