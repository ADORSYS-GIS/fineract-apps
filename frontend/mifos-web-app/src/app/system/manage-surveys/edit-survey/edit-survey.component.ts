...
export class EditSurveyComponent {
	 ...
	 */
	prepareQuestionDatas(
		questionForms: UntypedFormArray,
		questionDatas: Array<QuestionData>,
	) {
		questionDatas.forEach((questionData, idx) => {
			this.addQuestion();
			const questionForm: UntypedFormGroup = <UntypedFormGroup>(
				questionForms.at(idx)
			);
	...
	createSurveyForm() {
		this.surveyForm = this.formBuilder.group({
			...
			countryCode: [
				"",
				[Validators.required, Validators.pattern("^\\s*([A-Za-z]{2})?\\s*$")],
			],
			description: [""],
	...
	createResponseForm(): UntypedFormGroup {
		return this.formBuilder.group({
			...
			value: [
				"",
				[Validators.required, Validators.pattern("^\\s*[-]?\\d{0,4}\\s*$")],
			],
			sequenceNo: [""],
		});
	}
	...
	updateSequenceNumber() {
		for (
			let questionIndex = 0;
			questionIndex < this.questionDatas.length;
			questionIndex++
		) {
			this.questionDatas
				.at(questionIndex)
