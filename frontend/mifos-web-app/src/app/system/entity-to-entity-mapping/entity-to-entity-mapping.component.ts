...
export class EntityToEntityMappingComponent implements OnInit {
		...
		const formfields: FormfieldBase[] = [
			new SelectBase({
				...
				label: this.translateService.instant(
					`labels.inputs.${this.firstMappingEntity}`,
				),
			...
			new SelectBase({
				controlName: "toId",
				label: this.translateService.instant(
					`labels.text.${this.secondMappingEntity}`,
				),
		...
		const data = {
			title:
				...
				this.translateService.instant(
					"labels.heading.Entity to Entity Mapping",
				),
			layout: { addButtonText: "Confirm" },
			formfields: formfields,
		};
		const addEntitytoEntityMappingDialogRef = this.dialog.open(
			FormDialogComponent,
			{ data },
		);
		addEntitytoEntityMappingDialogRef
			.afterClosed()
			.subscribe((response: any) => {
				if (response.data) {
	...
	showEditScreen(selectedMap: number, selectedType: number) {
		...
		const editEntitytoEntityMappingDialogRef = this.dialog.open(
			FormDialogComponent,
			{ data },
		);
		editEntitytoEntityMappingDialogRef
			.afterClosed()
			.subscribe((response: any) => {
				if (response.data) {
	...
	submitNew(addMappingForm: any) {
		...
		this.systemService
			.createMapping(this.relId, newMappingData)
			.subscribe((response: any) => {
	...
	submitEdit(editMappingForm: any) {
		...
		newMappingData.locale = this.settingsService.language.code;
		this.systemService
			.editMapping(this.mapIdToEdit, newMappingData)
			.subscribe((response: any) => {
				this.showFilteredData();
			});
	}
	...
	delete(id: number) {
		const deleteNoteDialogRef = this.dialog.open(DeleteDialogComponent, {
			data: {
				deleteContext:
					this.translateService.instant("labels.inputs.Mapping id") + " " + id,
			},
		});
