...
export class CreateDataTableComponent implements OnInit, AfterViewInit {
	...
	@ViewChild("templateDataTableFormRef")
	templateDataTableFormRef: TemplateRef<any>;
	...
	ngOnInit() {
		...
		this.setColumns();
		this.dataTableForm.controls.apptableName.valueChanges.subscribe(
			(value: any) => {
	...
	editColumn(column: any) {
		...
		editColumnDialogRef.afterClosed().subscribe((response: any) => {
			if (response !== "") {
				this.columnData[
					this.columnData.findIndex(
						(newColumn) => newColumn.columnName === column.name,
					)
				] = {
					columnName: response.name,
					columnDisplayType: response.type,
	...
	deleteColumn(column: any) {
		const deleteColumnDialogRef = this.dialog.open(DeleteDialogComponent, {
			data: {
				deleteContext:
					this.translateService.instant("labels.inputs.Column") +
					" " +
					column.name,
			},
		});
	...
	submit() {
		...
		this.systemService.createDataTable(payload).subscribe((response: any) => {
			if (this.configurationWizardService.showDatatablesForm === true) {
				...
				this.router.navigate(["../", response.resourceIdentifier], {
	 ...
	 */
	openDialog() {
		const continueSetupDialogRef = this.dialog.open(
