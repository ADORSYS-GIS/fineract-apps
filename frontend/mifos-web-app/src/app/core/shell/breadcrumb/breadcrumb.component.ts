				...
				childrenRoutes.forEach((route) => {
					...
					if (hasData) {
						if (
							...
							route.snapshot.data[routeResolveBreadcrumb].forEach(
								(property: any) => {
							...
							if (routeData.breadcrumb === "Clients") {
								breadcrumbLabel = this.printableValue(
									routeData.clientViewData.displayName,
								);
								currentUrl += `/general`;
								...
								breadcrumbLabel =
									this.printableValue(
										routeData.loanDetailsData.loanProductName,
									) +
									" (" +
								...
								const savingsProductName =
									routeData.savingsAccountData?.savingsProductName ?? "";
								const accountNo = routeData.savingsAccountData?.accountNo ?? "";
								breadcrumbLabel =
									this.printableValue(savingsProductName) +
									(accountNo ? " (" + accountNo + ")" : "");
								...
								routeData.breadcrumb === "Recurring Deposit Products"
							) {
							...
							route.snapshot.data.hasOwnProperty(routeDataBreadcrumb)
						) {
