...
export class RolePermissionMapperService {
	...
	private rolePermissionMap: Map<string, string[]> = new Map([
		...
		["Super User", ["ALL_FUNCTIONS"]],
		["Super user", ["ALL_FUNCTIONS"]], // Handle case variation
		["admin", ["ALL_FUNCTIONS"]],
		...
		["readonly", ["ALL_FUNCTIONS_READ"]],
		["read-only", ["ALL_FUNCTIONS_READ"]],
	]);
	...
	mapRolesToPermissions(roles: any[]): string[] {
		if (!roles || !Array.isArray(roles)) {
			console.warn(
				"[RolePermissionMapper] No roles provided or roles is not an array",
			);
			return [];
		}
		...
		roles.forEach((role) => {
			...
			if (!roleName) {
				console.warn(
					"[RolePermissionMapper] Role object missing name property:",
					role,
				);
				return;
			}
			...
			if (rolePerms && rolePerms.length > 0) {
				console.log(
					`[RolePermissionMapper] Mapped role "${roleName}" to permissions:`,
					rolePerms,
				);
				rolePerms.forEach((perm) => permissions.add(perm));
			} else {
				console.warn(
					`[RolePermissionMapper] No permission mapping found for role: "${roleName}"`,
				);
			}
		});
	...
	private findRolePermissions(roleName: string): string[] | undefined {
		...
		for (const [key, value] of this.rolePermissionMap.entries()) {
	...
	addRoleMapping(roleName: string, permissions: string[]): void {
		...
		console.log(
			`[RolePermissionMapper] Added/updated role mapping for "${roleName}":`,
			permissions,
		);
	}
