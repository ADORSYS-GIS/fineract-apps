...
export class Datatables {
	...
	public isSystemColumn(columnName: string): boolean {
		return (
			this.systemFields.includes(columnName) ||
			this.entitiesIdFields.includes(columnName)
		);
	}
	...
	public getInputName(datatableInput: any): string {
		if (
			datatableInput.columnName &&
			datatableInput.columnName.includes("_cd_")
		) {
	...
	public isNumeric(columnType: string): boolean {
		return (
			this.isColumnType(columnType, "INTEGER") ||
			this.isColumnType(columnType, "DECIMAL")
		);
	}

	public isDate(columnType: string): boolean {
		return (
			this.isColumnType(columnType, "DATE") ||
			this.isColumnType(columnType, "DATETIME")
		);
	}
