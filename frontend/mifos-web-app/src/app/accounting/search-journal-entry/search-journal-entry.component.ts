...
export class SearchJournalEntryComponent implements OnInit, AfterViewInit {
	...
	applyFilter(filterValue: string, property: string) {
		...
		const findIndex = this.filterJournalEntriesBy.findIndex(
	...
	setFilteredGlAccounts() {
		this.filteredGLAccountData = this.glAccount.valueChanges.pipe(
			startWith(""),
			map((glAccount: any) =>
				typeof glAccount === "string"
					? glAccount
					: glAccount.name + " (" + glAccount.glCode + ")",
			),
			map((glAccount: string) =>
				glAccount
					? this.filterGLAccountAutocompleteData(glAccount)
					: this.glAccountData,
			),
		);
	}
	...
	private filterGLAccountAutocompleteData(glAccount: string): any {
		return this.glAccountData.filter((option: any) =>
			(option.name + " (" + option.glCode + ")")
				.toLowerCase()
				.includes(glAccount.toLowerCase()),
		);
	}
