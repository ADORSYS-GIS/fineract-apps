...
export class EditRuleComponent implements OnInit {
	...
	constructor(
		...
		this.route.data.subscribe(
			(data: { accountingRulesTemplate: any; accountingRule: any }) => {
				...
				this.creditTagData =
					data.accountingRulesTemplate.allowedCreditTagOptions;
	...
	setAccountingRulesForm() {
		this.accountingRuleForm
			.get("debitRuleType")
			.valueChanges.subscribe((debitRuleType) => {
		...
		this.accountingRuleForm
			.get("creditRuleType")
			.valueChanges.subscribe((creditRuleType) => {
		...
		if (this.accountingRule.debitAccounts) {
			...
			this.accountingRuleForm
				.get("debitTags")
				.setValue(
					this.accountingRule.debitTags.map((debitTag: any) => debitTag.tag.id),
				);
			this.accountingRuleForm
				.get("allowMultipleDebitEntries")
		...
		if (this.accountingRule.creditAccounts) {
			...
			this.accountingRuleForm
				.get("creditTags")
				.setValue(
					this.accountingRule.creditTags.map(
						(creditTag: any) => creditTag.tag.id,
					),
				);
			this.accountingRuleForm
				.get("allowMultipleCreditEntries")
