...
export class ClosingEntriesComponent implements OnInit, AfterViewInit {
	...
	@ViewChild("templateButtonCreateClosure")
	templateButtonCreateClosure: TemplateRef<any>;
	...
	constructor(
		...
		this.route.data.subscribe(
			(data: { offices: any; glAccountClosures: any }) => {
	...
	ngAfterViewInit() {
		...
		if (this.configurationWizardService.showClosingEntriesList === true) {
			setTimeout(() => {
				this.showPopover(
					this.templateClosuresTable,
					this.closuresTable.nativeElement,
					"top",
					true,
				);
			});
