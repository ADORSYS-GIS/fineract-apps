...
export class ViewProvisioningEntryComponent implements OnInit, AfterViewInit {
	...
	filterPredicate(data: any, filterValue: any) {
		return (
			...
			data.productName.toLowerCase().indexOf(filterValue["productName"]) !==
				-1 &&
			data.categoryName.toLowerCase().indexOf(filterValue["categoryName"]) !==
				-1
		);
	}
	...
	setProvisioningEntryEntries() {
		this.dataSource = new MatTableDataSource(
			this.provisioningEntryEntries.pageItems,
		);
	...
	setFilteredOffices() {
		this.filteredOfficeData = this.officeName.valueChanges.pipe(
			...
			map((officeName: string) =>
				officeName
					? this.filterOfficeAutocompleteData(officeName)
					: this.officeData,
			),
		);
	}
	...
	setFilteredLoanProducts() {
		this.filteredLoanProductData = this.loanProduct.valueChanges.pipe(
			startWith(""),
			map((loanProduct: any) =>
				typeof loanProduct === "string" ? loanProduct : loanProduct.name,
			),
	...
	setFilteredProvisioningCategories() {
		this.filteredProvisioningCategoryData =
			this.provisioningCategory.valueChanges.pipe(
				startWith(""),
				...
				map((provisioningCategoryName: string) =>
					provisioningCategoryName
						? this.filterProvisioningCategoryAutocompleteData(
								provisioningCategoryName,
							)
	...
	private filterLoanProductAutocompleteData(loanProductName: string): any {
		return this.loanProductData.filter((loanProduct: any) =>
			loanProduct.name
				.toLowerCase()
				.includes(loanProductName.toLocaleLowerCase()),
		);
	}
	 ...
	 */
	private filterProvisioningCategoryAutocompleteData(
		provisioningCategoryName: string,
	): any {
		...
		this.accountingService
			...
			.subscribe((response: any) => {
				this.router.navigate(
					["../../journal-entries/view", response.resourceId],
