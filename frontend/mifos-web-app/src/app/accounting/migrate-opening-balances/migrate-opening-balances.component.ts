...
export class MigrateOpeningBalancesComponent implements OnInit, AfterViewInit {
	...
	createOpeningBalancesForm() {
		...
		this.openingBalancesForm.controls.currencyCode.valueChanges.subscribe(
			(value: string) => {
	...
	retrieveOpeningBalances() {
		this.accountingService
			...
			.subscribe((openingBalancesData: any) => {
				const entry = this.openingBalancesForm.get(
					"glAccountEntries",
				) as UntypedFormArray;

				openingBalancesData.glAccounts =
					openingBalancesData.assetAccountOpeningBalances.concat(
	...
	glAccountTypeLabel(accountType: string): string {
		return this.translateService.instant(
			"labels.inputs.accounting." + accountType,
		);
	}
}
