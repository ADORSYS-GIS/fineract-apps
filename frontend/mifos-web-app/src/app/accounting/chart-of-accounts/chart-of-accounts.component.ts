...
export class ChartOfAccountsComponent implements AfterViewInit, OnInit {
	...
	constructor(
		...
		this.nestedTreeControl = new NestedTreeControl<GLAccountNode>(
			this._getChildren,
		);
	...
	ngOnInit() {
		...
		this.glAccountTreeService.treeDataChange.subscribe(
			(glAccountTreeData: GLAccountNode[]) => {
	...
	ngAfterViewInit() {
		...
		if (this.configurationWizardService.showChartofAccountsList === true) {
			setTimeout(() => {
				this.showPopover(
					this.templateAccountsTable,
					this.accountsTable.nativeElement,
					"top",
					true,
				);
			});
