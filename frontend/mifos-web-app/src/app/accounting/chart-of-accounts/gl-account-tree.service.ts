...
export class GlAccountTreeService {
	...
	buildGLAccountTree(glAccountData: GLAccount[]): GLAccountNode[] {
		...
		glAccountTree[0].children.push(
			new GLAccountNode(
				this.translateService.instant("labels.inputs.accounting.ASSET"),
			),
		);
		glAccountTree[0].children.push(
			new GLAccountNode(
				this.translateService.instant("labels.inputs.accounting.EQUITY"),
			),
		);
		glAccountTree[0].children.push(
			new GLAccountNode(
				this.translateService.instant("labels.inputs.accounting.EXPENSE"),
			),
		);
		glAccountTree[0].children.push(
			new GLAccountNode(
				this.translateService.instant("labels.inputs.accounting.INCOME"),
			),
		);
		glAccountTree[0].children.push(
			new GLAccountNode(
				this.translateService.instant("labels.inputs.accounting.LIABILITY"),
			),
		);
		...
		for (const glAccount of glAccountData) {
			if (glAccount.parentId === 0) {
				...
				if (glAccounts[glAccount.parentId]) {
					glAccounts[glAccount.parentId].children.push(
