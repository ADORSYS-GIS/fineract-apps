	...
	setGLAccountForm() {
		this.glAccountForm.get("type").valueChanges.subscribe((accountTypeId) => {
			switch (accountTypeId) {
				...
				case 2:
					this.parentData =
						this.chartOfAccountsData.liabilityHeaderAccountOptions;
				...
				case 5:
					this.parentData =
						this.chartOfAccountsData.expenseHeaderAccountOptions;
			...
			.subscribe((response: { step: number }) => {
				if (response.step === 1) {
					...
					this.router.navigate([
						"/accounting/chart-of-accounts/gl-accounts/create",
					]);
