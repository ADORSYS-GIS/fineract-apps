...
export class AssetTransferLoanComponent implements OnInit {
	...
	constructor(
		...
		this.command =
			actionName === "Sell Loan" ? this.SALE_COMMAND : this.BUYBACK_COMMAND;
	}
	...
	submit() {
		...
		if (saleLoanFormData.settlementDate instanceof Date) {
			saleLoanFormData.settlementDate = this.dateUtils.formatDate(
				prevsettlementDate,
				dateFormat,
			);
		}
		...
		this.externalAssetOwnerService
			...
			.subscribe((response: any) => {
				this.router.navigate(["../../external-asset-owner"], {
