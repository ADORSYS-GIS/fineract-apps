...
export class DisburseToSavingsAccountComponent implements OnInit {
	...
	ngOnInit() {
		...
		this.loanService
			.getLoanDelinquencyDataForTemplate(loanId)
			.subscribe((delinquencyData: any) => {
	...
	setDisbursementToSavingsForm() {
		...
		if (this.dataObject.fixedEmiAmount) {
			this.disbursementForm.addControl(
				"fixedEmiAmount",
				new UntypedFormControl(this.dataObject.fixedEmiAmount, [
					Validators.required,
				]),
			);
		}
	...
	submit() {
		...
		const prevActualDisbursementDate: Date =
			this.disbursementForm.value.actualDisbursementDate;
		if (disbursementLoanFormData.actualDisbursementDate instanceof Date) {
			disbursementLoanFormData.actualDisbursementDate =
				this.dateUtils.formatDate(prevActualDisbursementDate, dateFormat);
		}
		const data = {
