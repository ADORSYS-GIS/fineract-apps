...
export class AddInterestPauseComponent implements OnInit {
	...
	submit() {
		...
		if (interestPauseLoanFormData.startDate instanceof Date) {
			interestPauseLoanFormData.startDate = this.dateUtils.formatDate(
				startDate,
				dateFormat,
			);
		}
		...
		if (interestPauseLoanFormData.endDate instanceof Date) {
			interestPauseLoanFormData.endDate = this.dateUtils.formatDate(
				endDate,
				dateFormat,
			);
		}
		...
		this.loanService
			.addInterestPauseToLoan(this.loanId, data)
			.subscribe((response: any) => {
				this.router.navigate(["../../term-variations"], {
