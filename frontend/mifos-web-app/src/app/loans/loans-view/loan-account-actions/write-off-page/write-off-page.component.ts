...
export class WriteOffPageComponent implements OnInit {
	...
	submit() {
		...
		if (writeOffFormData.transactionDate instanceof Date) {
			writeOffFormData.transactionDate = this.dateUtils.formatDate(
				prevTransactionDate,
				dateFormat,
			);
		}
		if (
			writeOffFormData.writeoffReasonId === null ||
			writeOffFormData.writeoffReasonId === ""
		) {
		...
		delete data.amount;
		this.loanService
			.submitLoanActionButton(this.loanId, data, "writeoff")
