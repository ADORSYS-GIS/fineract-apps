...
export class LoanReagingComponent implements OnInit {
	...
	ngOnInit(): void {
		...
		this.reAgeInterestHandlingOptions =
			this.dataObject.reAgeInterestHandlingOptions;
	...
	createReagingLoanForm() {
		this.reagingLoanForm = this.formBuilder.group({
			...
			reAgeInterestHandling: [this.reAgeInterestHandlingOptions[0]],
			note: "",
			externalId: "",
	...
	submit(): void {
		...
		if (reagingLoanFormData.startDate instanceof Date) {
			reagingLoanFormData.startDate = this.dateUtils.formatDate(
				startDate,
				dateFormat,
			);
		}
