...
export class ForeclosureComponent implements OnInit {
	...
	createforeclosureForm() {
		this.foreclosureForm = this.formBuilder.group({
			...
			outstandingInterestPortion: [
				{ value: this.dataObject.interestPortion || 0, disabled: true },
			],
		...
		this.loanService
			...
			.subscribe((response: any) => {
				...
				this.foreclosureForm.patchValue({
					...
					outstandingPenaltyChargesPortion:
						this.foreclosuredata.penaltyChargesPortion,
				});
			});
	}
	...
	submit() {
		...
		this.loanService
			.loanForclosureData(this.loanId, data)
			.subscribe((response: any) => {
