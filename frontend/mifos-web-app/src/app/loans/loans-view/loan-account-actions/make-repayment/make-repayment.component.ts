...
export class MakeRepaymentComponent implements OnInit {
	...
	isCapitalizedIncome(): boolean {
		return ["capitalizedIncome", "capitalizedIncomeAdjustment"].includes(
			this.command,
		);
	}

	isBuyDownFee(): boolean {
		return ["buyDownFee"].includes(this.command);
	}

	showInterestRefundCheckbox(): boolean {
		const code = this.dataObject?.type?.code?.toLowerCase() || "";
		return (
			code.includes("merchantissuedrefund") || code.includes("payoutrefund")
		);
	}
	...
	submit() {
		...
		const prevTransactionDate: Date =
			this.repaymentLoanForm.value.transactionDate;
