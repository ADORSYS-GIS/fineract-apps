...
export class LoanTrancheDetailsComponent implements OnInit {
	...
	constructor(
		...
		this.route.parent.data.subscribe((data: { loanDetailsData: any }) => {
			...
			this.disallowExpectedDisbursements =
				this.loanDetails.disallowExpectedDisbursements || false;
	...
	ngOnInit() {
		...
		this.disbursementDataSource =
			this.loanServices.getLoanDisbursementDetailsData();
	...
	showAddTrancheButtons() {
		...
		if (
			this.totalMultiDisbursed === this.currentPrincipalAmount ||
			this.return === false
		) {
			return false;
		}
	...
	addDisbursementDataEntry() {
		...
		const data = {
			...
			formfields: this.buildForm(
				new Date(),
				this.currentPrincipalAmount - this.totalMultiDisbursed,
			),
		};
		...
		disbursementDialogRef.afterClosed().subscribe((response: any) => {
			if (response.data) {
				...
				if (
					...
					this.loanServices.saveLoanDisbursementDetailsData(
						this.disbursementDataSource,
					);
					this.pristine = false;
				}
	...
	editDisbursementDataEntry(index: number) {
		const principal: number =
			this.disbursementDataSource[index]["principal"] * 1;
		...
		disbursementDialogRef.afterClosed().subscribe((response: any) => {
			if (response.data) {
				...
				if (
					this.totalMultiDisbursed - principal + newPrincipal <=
					this.currentPrincipalAmount
				) {
