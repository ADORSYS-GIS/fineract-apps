	...
	constructor(
		...
		this.route.data.subscribe((data: { loansAccountTransaction: any }) => {
			...
			if (this.transactionData.transactionRelations) {
				this.transactionRelations.data =
					this.transactionData.transactionRelations;
				this.existTransactionRelations =
					this.transactionData.transactionRelations.length > 0;
				...
				this.amountRelationsAllowed =
					this.transactionData.amount - amountRelations;
			...
			undoTransactionAccountDialogRef
				...
				.subscribe((response: { confirm: any }) => {
					if (response.confirm) {
						...
						const command = this.isWriteOff(this.transactionType)
							? "undowriteoff"
							: "undo";
						const transactionId =
							command === "undowriteoff" ? null : this.transactionData.id;
						this.loansService
			...
			if (response.data) {
				if (response.data.value.amount <= this.amountRelationsAllowed) {
					...
					this.alertService.alert({
						type: "BusinessRule",
						message:
							"Chargeback amount must be lower or equal to: " +
							this.amountRelationsAllowed,
					});
				}
			}
