...
export class LoanTermVariationsTabComponent {
	...
	deleteInterestPause(variation: any) {
		const deleteStandingInstructionDialogRef = this.dialog.open(
			...
			{
				data: {
					deleteContext: `interest pause from ${variation.startDate} to ${variation.endDate}`,
				},
			},
		);
	...
	updateInterestPause(variation: any) {
		...
		const editDialogRef = this.dialog.open(FormDialogComponent, {
			data,
			width: "50rem",
		});
		editDialogRef.afterClosed().subscribe((response: any) => {
			if (response.data) {
				if (response.data.value.startDate <= response.data.value.endDate) {
					...
					const payload = {
						startDate: this.dates.formatDate(
							response.data.value.startDate,
							dateFormat,
						),
						endDate: this.dates.formatDate(
							response.data.value.endDate,
							dateFormat,
						),
						locale,
						dateFormat,
	...
	private reload() {
		...
		this.router
			.navigateByUrl(`/clients/${this.clientId}/loans-accounts`, {
