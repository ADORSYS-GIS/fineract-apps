...
export class RescheduleLoanTabComponent {
	...
	manageRequest(request: any, command: string): void {
		const approveLoanRescheduleDialogRef = this.dialog.open(
			ConfirmationDialogComponent,
			{
				data: {
					heading:
						`${command}` +
						this.translateService.instant("labels.heading.Loan Reschedule"),
					dialogContext:
						...
						`${command}` +
						this.translateService.instant(
							"labels.dialogContext.the Loan Reschedule",
						) +
						`${request.id}`,
				},
			},
		);
		approveLoanRescheduleDialogRef
			.afterClosed()
