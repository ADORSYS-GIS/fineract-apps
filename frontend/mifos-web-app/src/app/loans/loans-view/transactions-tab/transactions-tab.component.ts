...
export class TransactionsTabComponent implements OnInit {
		...
		if (hideAccrual || hideReversed) {
			transactions = this.transactionsData.filter((t: LoanTransaction) => {
				return (
					...
					!(hideAccrual && this.isAccrualKindOf(t.type))
				);
			});
		}
	...
	showTransaction(transactionsData: LoanTransaction): boolean {
		return [
			1, 2, 4, 9, 20, 21, 22, 23, 26, 28, 29, 30, 31, 32, 33, 35, 36, 37, 38,
			40, 41, 42,
	...
	loanTransactionColor(transaction: LoanTransaction): string {
		...
		if (
			...
			transaction.transactionRelations.length > 0
		) {
			return "linked";
		}
	...
	undoReAgeOrReAmortize(transaction: LoanTransaction): void {
		...
		undoTransactionAccountDialogRef.afterClosed().subscribe((response: any) => {
			if (response.confirm) {
				const undoCommand =
					actionName === "Re-Age" ? "undoReAge" : "undoReAmortize";
				this.loansService
					.executeLoansAccountTransactionsCommand(
						String(this.loanId),
						undoCommand,
	...
	private isChargeOff(transactionType: LoanTransactionType): boolean {
		return (
			...
			transactionType.code === "loanTransactionType.chargeOff"
		);
	}
	...
	isWriteOff(transactionType: LoanTransactionType): boolean {
		return (
			...
			transactionType.code === "loanTransactionType.writeOff"
		);
	}
	...
	private isDownPayment(transactionType: LoanTransactionType): boolean {
		return (
			...
			transactionType.code === "loanTransactionType.downPayment"
		);
	}

	private isReAge(transactionType: LoanTransactionType): boolean {
		return (
			transactionType.reAge ||
			transactionType.code === "loanTransactionType.reAge"
		);
	}

	private isReAmortize(transactionType: LoanTransactionType): boolean {
		return (
			transactionType.reAmortize ||
			transactionType.code === "loanTransactionType.reAmortize"
		);
	}
	...
	private isBuyDownFeeAmortization(
	...
	private isCapitalizedIncomeAmortization(
		transactionType: LoanTransactionType,
	): boolean {
		return (
	...
	isBuyDownFee(transactionType: LoanTransactionType): boolean {
		return (
			transactionType.buyDownFee ||
			transactionType.code === "loanTransactionType.buyDownFee"
		);
	}

	viewJournalEntry(transactionType: LoanTransactionType): boolean {
		return !(
			this.isReAmortize(transactionType) || this.isReAge(transactionType)
		);
	}
	...
	canCreateInterestRefund(transaction: LoanTransaction): boolean {
		...
		const isRefundType =
			type.includes("payoutrefund") || type.includes("merchantissuedrefund");
		if (!isRefundType) return false;
		if (transaction.manuallyReversed) return false;
		if (
			transaction.transactionRelations &&
			transaction.transactionRelations.some(
				(r) => r.relationType === "INTEREST_REFUND",
			)
		)
			return false;
		return true;
			...
			.subscribe((template: any) => {
				...
				const data = {
					title: this.translateService.instant(
						"labels.buttons.Create Interest Refund",
					),
					layout: {
						addButtonText: this.translateService.instant(
							"labels.buttons.Create Interest Refund",
						),
					},
		...
		this.loansService
			...
			.subscribe((response: any) => {
				...
				if (response.amount == 0) {
					...
					const chargebackDialogRef = this.dialog.open(FormDialogComponent, {
						data,
					});
					chargebackDialogRef
						.afterClosed()
	...
	buyDownFeeAdjustmentTransaction(transaction: LoanTransaction) {
		...
		this.loansService
			...
			.subscribe((response: any) => {
				...
				if (response.amount == 0) {
					this.displayAlertMessage(
						"Buy Down Fee amount already adjusted",
						transaction.amount,
					);
				} else {
					const transactionAmount = response.amount || transaction.amount;
