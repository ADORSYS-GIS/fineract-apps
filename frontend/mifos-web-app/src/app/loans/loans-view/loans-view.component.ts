...
export class LoansViewComponent implements OnInit {
	...
	constructor(
		...
		this.route.data.subscribe(
			(data: {
				loanDetailsData: any;
				loanDatatables: any;
				loanArrearsDelinquencyConfig: any;
			}) => {
				this.loanDetailsData = data.loanDetailsData;
				this.loanDatatables = data.loanDatatables;
				this.loanDisplayArrearsDelinquency =
					data.loanArrearsDelinquencyConfig.value || 0;
				this.loanStatus = this.loanDetailsData.status;
				this.loanSubStatus =
					this.loanDetailsData.subStatus === undefined
						? null
						: this.loanDetailsData.subStatus;
				this.currency = this.loanDetailsData.currency;
				loansService.saveLoanDisbursementDetailsData(
					this.loanDetailsData.disbursementDetails,
				);
	...
	setConditionalButtons() {
		...
		if (this.status === "Submitted and pending approval") {
			this.buttonConfig.addOption({
				name: this.loanDetailsData.loanOfficerName
					? "Change Loan Officer"
					: "Assign Loan Officer",
				icon: "user-tie",
			...
			if (
				this.loanDetailsData.canDisburse ||
			...
			if (
				LoanProducts.isAdvancedPaymentAllocationStrategy(
					this.loanDetailsData.transactionProcessingStrategyCode,
				)
			) {
				this.buttonConfig.addButton({
					name: "Reschedule",
	...
	private recoverFromGuarantor() {
		const recoverFromGuarantorDialogRef = this.dialog.open(
			ConfirmationDialogComponent,
			{
				data: {
					heading: this.translateService.instant(
						"labels.heading.Recover from Guarantor",
					),
		...
		recoverFromGuarantorDialogRef.afterClosed().subscribe((response: any) => {
			if (response.confirm) {
				this.loansService
					.loanActionButtons(this.loanId, "recoverGuarantees")
					.subscribe(() => {
						this.reload();
					});
			}
	...
	private reload() {
		...
		this.router
			.navigateByUrl(`/clients/${clientId}/loans-accounts`, {
