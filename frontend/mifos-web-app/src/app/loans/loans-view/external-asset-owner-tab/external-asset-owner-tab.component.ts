...
export class ExternalAssetOwnerTabComponent implements OnInit {
	...
	constructor(
		...
		this.route.data.subscribe(
			(data: { loanTransfersData: any; activeTransferData: any }) => {
				this.loanTransfersData = data.loanTransfersData.empty
					? []
					: data.loanTransfersData.content;
	...
	ngOnInit(): void {
		...
		if (this.loanTransfersData.length > 0) {
			this.currentItem =
				this.loanTransfersData[this.loanTransfersData.length - 1];
		}
	}
