...
export class ExternalAssetOwnerService {
	...
	executeExternalAssetOwnerTransferCommand(
		...
		const httpParams = new HttpParams().set("command", command);
		return this.http.post(`${this.basePath}/transfers/${transferId}`, data, {
			params: httpParams,
		});
	}
	...
	retrieveExternalAssetOwnerActiveTransfer(loanId: string): Observable<any> {
		const httpParams = new HttpParams().set("loanId", loanId);
		return this.http.get(`${this.basePath}/transfers/active-transfer`, {
			params: httpParams,
		});
	}
	 ...
	 * @returns {Observable<any>}
	 */
	retrieveExternalAssetOwnerTransferJournalEntries(
		transferId: string,
	): Observable<any> {
		return this.http.get(
			`${this.basePath}/transfers/${transferId}/journal-entries`,
		);
	}
