...
export class LoanActionButtonResolver {
	...
	resolve(route: ActivatedRouteSnapshot): Observable<any> {
		const loanId =
			route.paramMap.get("loanId") || route.parent.paramMap.get("loanId");
		const loanActionButton = route.paramMap.get("action");
		if (
			loanActionButton === "Assign Loan Officer" ||
			loanActionButton === "Change Loan Officer"
		) {
			...
			return this.loansService.getLoanActionTemplate(
				loanId,
