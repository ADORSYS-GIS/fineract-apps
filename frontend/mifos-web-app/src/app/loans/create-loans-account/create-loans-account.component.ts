...
export class CreateLoansAccountComponent {
	...
	get loansAccountFormValid() {
		return (
			this.loansAccountDetailsForm.valid && this.loansAccountTermsForm.valid
		);
	}

	get loansSavingsAccountLinked() {
		return this.loansAccountDetailsStep.loansAccountDetailsForm.get(
			"linkAccountId",
		).value;
	}
	...
	submit() {
		...
		if (
			...
			this.loanDatatables.forEach(
				(loanDatatable: LoansAccountDatatableStepComponent) => {
		...
		this.loansService.createLoansAccount(payload).subscribe((response: any) => {
			this.router.navigate(["../", response.resourceId, "general"], {
