...
export class LoansAccountTermsStepComponent implements OnInit, OnChanges {
	...
	ngOnChanges() {
		if (this.loansAccountProductTemplate) {
			...
			this.productEnableDownPayment =
				this.loansAccountTermsData.product.enableDownPayment;
			this.enableIncomeCapitalization =
				this.loansAccountTermsData.product.enableIncomeCapitalization;
			this.enableBuyDownFee =
				this.loansAccountTermsData.product.enableBuyDownFee;
			this.isProgressive =
			...
			this.loansAccountTermsForm.patchValue({
				...
				loanTermFrequencyType:
					this.loansAccountTermsData.termPeriodFrequencyType.id,
				...
				repaymentFrequencyType:
					this.loansAccountTermsData.repaymentFrequencyType.id,
				...
				isFloatingInterestRate: this.loansAccountTermsData
					.isLoanProductLinkedToFloatingRate
					? false
					: "",
				interestCalculationPeriodType:
					this.loansAccountTermsData.interestCalculationPeriodType.id,
				...
				graceOnPrincipalPayment:
					this.loansAccountTermsData.graceOnPrincipalPayment,
				graceOnInterestPayment:
					this.loansAccountTermsData.graceOnInterestPayment,
				graceOnArrearsAgeing: this.loansAccountTermsData.graceOnArrearsAgeing,
				graceOnInterestCharged:
					this.loansAccountTermsData.graceOnInterestCharged,
				...
				interestRateDifferential:
					this.loansAccountTermsData.interestRateDifferential,
				multiDisburseLoan: this.loansAccountTermsData.multiDisburseLoan,
				interestRateFrequencyType:
					this.loansAccountTermsData.interestRateFrequencyType.id,
				balloonRepaymentAmount:
					this.loansAccountTermsData.balloonRepaymentAmount,
				interestRecognitionOnDisbursementDate:
					this.loansAccountTermsData.interestRecognitionOnDisbursementDate ||
					false,
			});
			...
			if (
				...
				this.loansAccountTermsForm.removeControl(
					"interestRecognitionOnDisbursementDate",
				);
			}
			...
			if (this.loansAccountTermsData.disbursementDetails) {
				this.disbursementDataSource =
					this.loansAccountTermsData.disbursementDetails;
			...
			if (this.productEnableDownPayment) {
				const enableDownPayment =
					this.loansAccountTermsData["enableDownPayment"] === false
						? false
						: true;
				this.loansAccountTermsForm.addControl(
					"enableDownPayment",
			...
			const allowAttributeOverrides =
				this.loansAccountTermsData.product.allowAttributeOverrides;
	...
	allowAddDisbursementDetails() {
		return (
			...
			!this.loansAccountTermsData.disallowExpectedDisbursements
		);
	}
	...
	setLoanTermListener() {
		this.loansAccountTermsForm
			...
			.valueChanges.subscribe((numberOfRepayments) => {
				const repaymentEvery: number =
					this.loansAccountTermsForm.value.repaymentEvery;
		...
		this.loansAccountTermsForm
			...
			.valueChanges.subscribe((repaymentEvery) => {
				const numberOfRepayments: number =
					this.loansAccountTermsForm.value.numberOfRepayments;
				this.calculateLoanTerm(numberOfRepayments, repaymentEvery);
			});
		...
		if (this.loansAccountTermsData) {
			...
			if (this.loansAccountTermsData.product.fixedLength) {
				...
				this.loansAccountTermsForm.addControl(
					"fixedLength",
					new UntypedFormControl(
						this.loansAccountTermsData.product.fixedLength,
					),
				);
			} else {
				this.loansAccountTermsForm.addControl(
	...
	addDisbursementDataEntry() {
		const currentPrincipalAmount =
			this.loansAccountTermsForm.get("principalAmount").value;
			...
			if (response.data) {
				...
				if (this.totalMultiDisbursed + principal <= currentPrincipalAmount) {
					...
					this.isMultiDisbursedCompleted =
						this.totalMultiDisbursed === currentPrincipalAmount;
					this.pristine = false;
				}
			}
		});
	 ...
	 */
	addCollateral() {
		const addCollateralDialogRef = this.dialog.open(
		...
		addCollateralDialogRef.afterClosed().subscribe((response: any) => {
			if (response.data) {
				...
				this.totalCollateralValue +=
					(collateralData.type.pctToBase *
						collateralData.type.basePrice *
						collateralData.value) /
					100;
				this.collateralDataSource =
					this.collateralDataSource.concat(collateralData);
				this.collateralOptions = this.collateralOptions.filter(
					(user: any) =>
						user.collateralId !== response.data.value.collateral.collateralId,
				);
	...
	deleteCollateral(id: any) {
		...
		deleteCollateralDialogRef.afterClosed().subscribe((response: any) => {
			if (response.delete) {
				...
				this.totalCollateralValue -=
					(removed[0].type.pctToBase *
						removed[0].type.basePrice *
						removed[0].value) /
					100;
	 ...
	 */
	setOptions() {
		this.termFrequencyTypeData =
			this.loansAccountProductTemplate.termFrequencyTypeOptions;
		...
		this.interestTypeData =
			this.loansAccountProductTemplate.interestTypeOptions;
		this.amortizationTypeData =
			this.loansAccountProductTemplate.amortizationTypeOptions;
		...
		if (
			...
			this.transactionProcessingStrategyOptions =
				this.loansAccountProductTemplate.transactionProcessingStrategyOptions.filter(
					(cn: CodeName) =>
						!LoanProducts.isAdvancedPaymentAllocationStrategy(cn.code),
				);
