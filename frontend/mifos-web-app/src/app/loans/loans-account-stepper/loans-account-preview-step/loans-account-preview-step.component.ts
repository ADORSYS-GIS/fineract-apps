...
export class LoansAccountPreviewStepComponent implements OnChanges {
	...
	membersDisplayedColumns: string[] = ["id", "name", "purpose", "principal"];
	...
	ngOnChanges(changes: SimpleChanges): void {
		this.productEnableDownPayment =
			this.loansAccountProductTemplate.product.enableDownPayment;
		if (this.activeClientMembers) {
			...
			this.dataSource = new MatTableDataSource<any>(
				this.activeClientMembers
					...
					.map((member: any) => ({
						...member,
						purpose: this.loanPurposeOptions.find(
							(option) => option.id === member.loanPurposeId,
						)?.name,
					})),
			);
