...
export class LoansAccountChargesStepComponent implements OnInit, OnChanges {
	...
	ngOnChanges() {
		if (this.loansAccountProductTemplate) {
			this.loanPurposeOptions =
				this.loansAccountProductTemplate.loanPurposeOptions;
			...
			if (
				...
				this.loansAccountTemplate.charges
			) {
				this.chargesDataSource =
					this.loansAccountTemplate.charges.map((charge: any) => {
						return {
	...
	editChargeAmount(charge: any) {
		...
		editNoteDialogRef.afterClosed().subscribe((response: any) => {
			if (response.data) {
				...
				this.chargesDataSource.splice(
					this.chargesDataSource.indexOf(charge),
					1,
					newCharge,
				);
	...
	editChargeDate(charge: any) {
		...
		editNoteDialogRef.afterClosed().subscribe((response: any) => {
			if (response.data) {
				...
				const date = this.dateUtils.formatDate(
					response.data.value.date,
					dateFormat,
				);
	...
	editChargeFeeInterval(charge: any) {
		...
		editNoteDialogRef.afterClosed().subscribe((response: any) => {
			if (response.data) {
				const newCharge = {
					...charge,
					feeInterval: response.data.value.feeInterval,
				};
				this.chargesDataSource.splice(
					this.chargesDataSource.indexOf(charge),
					1,
		...
		return {
			charges: uniqueCharges.map((charge: any) => {
				...
				if (charge.feeInterval !== undefined)
					result.feeInterval = charge.feeInterval;
				if (charge.feeOnMonthDay !== undefined)
					result.feeOnMonthDay = charge.feeOnMonthDay;

				return result;
			}),
		};
	}
	private getUniqueCharges<
		T extends { id?: number | string; chargeId?: number | string },
	>(charges: T[]): T[] {
	...
	get selectedClientMembers() {
		return {
			selectedMembers: this.activeClientMembers.filter(
	...
	toggleSelect() {
		...
		this.selectAllItems =
			len === 0
				? false
				: this.activeClientMembers.filter((item: any) => item.selected)
						.length === len;
	}
}
