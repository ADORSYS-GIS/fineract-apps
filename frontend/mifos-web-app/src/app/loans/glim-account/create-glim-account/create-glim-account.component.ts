...
export class CreateGlimAccountComponent {
	...
	@ViewChildren("dtloan")
	loanDatatables: QueryList<LoansAccountDatatableStepComponent>;
	...
	constructor(
		...
		this.route.data.subscribe(
			(data: { loansAccountTemplate: any; groupsData: any }) => {
	...
	setTemplate($event: any) {
		...
		if (!!clientId) {
			this.clientService
				.getCollateralTemplate(clientId)
			...
			console.error(
				"No collateral data requested from Fineract, collateral might misbehave",
			);
		}
	...
	setData(client: any, totalLoan: number): any {
		...
		const data = {
			...
			submittedOnDate: this.dateUtils.formatDate(
				this.loansAccount.submittedOnDate,
				dateFormat,
			),
			dateFormat,
			// monthDayFormat,
	...
	submit() {
		...
		const memberSelected =
			this.loansActiveClientMembers?.selectedClientMembers?.selectedMembers ??
			[];
		if (!memberSelected.length) return;
		const gsimMemberIds = new Set(
			this.dataSource.map((m: any) => Number(m.id)),
		);
		for (const member of memberSelected) {
			...
			if (
				member.linkAccountId &&
				member.linkAccountOwnerId &&
				ownerId !== memberId
			) {
				this.i18nService
					.translate("errors.linkedSavingsAccountOwnership")
			...
			if (!gsimMemberIds.has(memberId)) {
				this.i18nService
					.translate("errors.clientNotInGSIM", { id: memberId })
					.subscribe((msg: string) => {
						this.notify({ defaultUserMessage: msg, errors: [] }, { memberId });
					});
				return;
		...
		this.loansService.createGlimAccount(data).subscribe((response: any) => {
			...
			if (body.glimId) {
				this.router.navigate(["../", body.glimId], { relativeTo: this.route });
			} else {
	...
	notify(body: any, context?: { [k: string]: unknown }) {
		...
		if (Array.isArray(body?.errors)) {
			for (const e of body.errors)
				parts.push(String(e?.developerMessage ?? ""));
		}
