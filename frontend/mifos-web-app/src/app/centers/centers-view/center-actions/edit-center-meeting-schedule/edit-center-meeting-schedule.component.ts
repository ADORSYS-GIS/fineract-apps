...
@Component({
	...
	imports: [...STANDALONE_SHARED_IMPORTS, DateFormatPipe],
})
export class EditCenterMeetingScheduleComponent implements OnInit {
	 ...
	 */
	submit() {
		const centerEditMeetingScheduleFormData =
			this.centerEditMeetingScheduleForm.value;
		const locale = this.settingsService.language.code;
		const dateFormat = this.settingsService.dateFormat;
		const reschedulebasedOnMeetingDates = true;
		const prevOldDate: Date = new Date(
			this.centerEditMeetingScheduleForm.value.presentMeetingDate,
		);
		if (centerEditMeetingScheduleFormData.startDate instanceof Date) {
			centerEditMeetingScheduleFormData.presentMeetingDate =
				this.dateUtils.formatDate(prevOldDate, dateFormat);
		}
		const prevNewDate: Date =
			this.centerEditMeetingScheduleForm.value.newMeetingDate;
		if (centerEditMeetingScheduleFormData.newMeetingDate instanceof Date) {
			centerEditMeetingScheduleFormData.newMeetingDate =
				this.dateUtils.formatDate(prevNewDate, dateFormat);
		}
		const data = {
