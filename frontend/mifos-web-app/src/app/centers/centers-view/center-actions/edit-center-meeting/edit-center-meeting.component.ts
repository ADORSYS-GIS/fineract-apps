...
@Component({
	...
	imports: [...STANDALONE_SHARED_IMPORTS, FaIconComponent, MatHint],
})
export class EditCenterMeetingComponent implements OnInit {
	...
	buildDependencies() {
		this.centerEditMeetingForm
			.get("frequency")
			.valueChanges.subscribe((frequency: any) => {
		...
		this.centerEditMeetingForm.patchValue({
			startDate:
				this.calendarTemplate.startDate &&
				new Date(this.calendarTemplate.startDate),
	...
	submit() {
		...
		if (centerEditMeetingFormData.startDate instanceof Date) {
			centerEditMeetingFormData.startDate = this.dateUtils.formatDate(
				prevStartDate,
				dateFormat,
			);
		}
