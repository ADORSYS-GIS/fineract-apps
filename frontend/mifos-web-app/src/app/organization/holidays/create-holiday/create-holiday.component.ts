...
export class CreateHolidayComponent implements OnInit {
	...
	checklistSelection = new SelectionModel<OfficeItemFlatNode>(
		true /* multiple */,
	);
	...
	constructor(
		...
		this.route.data.subscribe(
			(data: { offices: any; holidayTemplate: any }) => {
		...
		this.treeControl = new FlatTreeControl<OfficeItemFlatNode>(
			this.getLevel,
			this.isExpandable,
		);
	...
	hasNoContent = (_: number, _nodeData: OfficeItemFlatNode) =>
		_nodeData.item === "";
	...
	buildDependencies() {
		this.holidayForm
			...
			.valueChanges.subscribe((option: any) => {
				if (option === 2) {
	...
	submit() {
		...
		holidayFormData.fromDate = this.dateUtils.formatDate(
			prevFromDate,
			dateFormat,
		);
		...
		if (this.holidayForm.contains("repaymentsRescheduledTo")) {
			const prevRepaymentsRescheduledTo: Date =
				this.holidayForm.value.repaymentsRescheduledTo;
