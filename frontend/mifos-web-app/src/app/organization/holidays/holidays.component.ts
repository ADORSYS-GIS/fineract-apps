...
export class HolidaysComponent implements OnInit, AfterViewInit {
	...
	@ViewChild("templateButtonCreateHoliday")
	templateButtonCreateHoliday: TemplateRef<any>;
	/* Reference of filter */
	 ...
	 */
	onChangeOffice() {
		this.officeSelector.valueChanges.subscribe(
			(officeId = this.officeSelector.value) => {
				this.holidaysData = [];
				this.organizationService
					.getHolidays(officeId)
					.subscribe((holidays: any) => {
						this.holidaysData = holidays.filter(
							(holiday: any) => holiday.status.value !== "Deleted",
						);
						this.setHolidays();
					});
			},
