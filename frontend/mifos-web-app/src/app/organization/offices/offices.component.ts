...
export class OfficesComponent implements OnInit, AfterViewInit {
	...
	ngOnInit() {
		this.setOffices();
		this.officeTreeService.treeDataChange.subscribe(
			(officeTreeData: OfficeNode[]) => {
	...
	ngAfterViewInit() {
		...
		if (this.configurationWizardService.showOfficeTable === true) {
			setTimeout(() => {
				this.showPopover(
					this.templateTableOffices,
					this.tableOffices.nativeElement,
					"top",
					true,
				);
			});
