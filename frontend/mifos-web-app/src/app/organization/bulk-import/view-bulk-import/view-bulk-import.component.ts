...
export class ViewBulkImportComponent implements OnInit {
	...
	@ViewChild("importsTable", { static: true })
	importsTableRef: MatTable<Element>;
	 ...
	 */
	ngOnInit() {
		this.bulkImport = this.bulkImportsArray.find(
			(entry) => entry.name === this.bulkImport.name,
		);
	...
	downloadTemplate() {
		...
		this.organizationService
			...
			.subscribe((res: any) => {
				...
				const fileOfBlob = new File([blob], "template.xls", {
					type: contentType,
				});
