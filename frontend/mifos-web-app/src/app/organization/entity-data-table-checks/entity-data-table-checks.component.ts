...
export class EntityDataTableChecksComponent implements OnInit {
	...
	deleteEntityDataTableCheck(entityDataTableCheckId: string) {
		const deleteEntityDataTableCheckDialogRef = this.dialog.open(
			DeleteDialogComponent,
			{
				data: {
					deleteContext: `entity data table check ${entityDataTableCheckId}`,
				},
			},
		);
			...
			.subscribe((response: any) => {
				if (response.delete) {
					this.organizationService
						...
						.subscribe(() => {
							this.entityDataTableChecksData =
								this.entityDataTableChecksData.filter(
									(entityDataTableChecks: any) =>
										entityDataTableChecks.id !== entityDataTableCheckId,
								);
