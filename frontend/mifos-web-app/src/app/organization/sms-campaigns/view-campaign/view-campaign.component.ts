...
export class ViewCampaignComponent implements OnInit {
	...
	onTabChange($event: any) {
		...
		const tab = this.smsTabs.find(
			(entry: any) => selectedTabName === entry.label,
		);
	...
	closeCampaign() {
		...
		const closeCampaignDialogRef = this.dialog.open(FormDialogComponent, {
			data,
		});
		closeCampaignDialogRef.afterClosed().subscribe((response: any) => {
			if (response.data) {
				...
				const dataObject = {
					closureDate: this.dateUtils.formatDate(
						response.data.value.closureDate,
						dateFormat,
					),
					dateFormat,
					locale,
	...
	activateCampaign() {
		...
		const activateCampaignDialogRef = this.dialog.open(FormDialogComponent, {
			data,
		});
			...
			if (response.data) {
				...
				const dataObject = {
					activationDate: this.dateUtils.formatDate(
						response.data.value.activationDate,
						dateFormat,
					),
					dateFormat,
					locale,
	...
	reactivateCampaign() {
		...
		const reactivateCampaignDialogRef = this.dialog.open(FormDialogComponent, {
			data,
		});
	...
	deleteCampaign() {
		const deleteSmsCampaignDialogRef = this.dialog.open(DeleteDialogComponent, {
			data: {
				deleteContext: `sms campaing with id: ${this.smsCampaignData.id}`,
			},
		});
	...
	private reload() {
		...
		this.router
			.navigateByUrl(`/organization/sms-campaigns`, {
	...
	search() {
		...
		if (smsFormData.fromDate instanceof Date) {
			smsFormData.fromDate = this.dateUtils.formatDate(
				prevFromDate,
				dateFormat,
			);
		}
