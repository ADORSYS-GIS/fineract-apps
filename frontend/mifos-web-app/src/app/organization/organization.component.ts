...
export class OrganizationComponent implements AfterViewInit {
	...
	@ViewChild("templateAddEditCurrency")
	templateAddEditCurrency: TemplateRef<any>;
	...
	ngAfterViewInit() {
		...
		if (this.configurationWizardService.showDefineWorkingDays === true) {
			setTimeout(() => {
				this.showPopover(
					...
					this.workingDays.nativeElement,
					"bottom",
					true,
				);
			});
		...
		if (this.configurationWizardService.showManageFunds === true) {
			setTimeout(() => {
				this.showPopover(
					this.templateManageFunds,
					this.manageFunds.nativeElement,
					"bottom",
					true,
				);
			});
