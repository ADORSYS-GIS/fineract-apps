...
export class EditCashierComponent implements OnInit {
	...
	constructor(
		...
		this.route.data.subscribe(
			(data: { cashier: any; cashierTemplate: any }) => {
	...
	setEditChargeForm() {
		this.editCashierForm = this.formBuilder.group({
			...
			startDate: [
				this.cashierData.data.startDate &&
					new Date(this.cashierData.data.startDate),
				Validators.required,
			],
			endDate: [
				this.cashierData.data.endDate &&
					new Date(this.cashierData.data.endDate),
				Validators.required,
			],
	...
	submit() {
		...
		if (editCashierFormData.startDate instanceof Date) {
			editCashierFormData.startDate = this.dateUtils.formatDate(
				prevStartDate,
				dateFormat,
			);
		}
		if (editCashierFormData.endDate instanceof Date) {
			editCashierFormData.endDate = this.dateUtils.formatDate(
				prevEndDate,
				dateFormat,
			);
		}
		...
		this.organizationService
			.updateCashier(
				this.cashierData.data.tellerId,
				this.cashierData.data.id,
				data,
			)
