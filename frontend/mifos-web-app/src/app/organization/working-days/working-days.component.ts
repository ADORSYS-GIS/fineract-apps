...
@Component({
	...
	imports: [...STANDALONE_SHARED_IMPORTS, MatCheckbox],
})
export class WorkingDaysComponent implements OnInit, AfterViewInit {
	...
	@ViewChild("templateWorkingDaysFormRef")
	templateWorkingDaysFormRef: TemplateRef<any>;
	...
	createWorkingDaysForm() {
		...
		this.repaymentRescheduleTypeData =
			this.workingDaysData.repaymentRescheduleOptions;
		this.workingDaysForm = this.formBuilder.group({
			...
			repaymentRescheduleType: [
				this.workingDaysData.repaymentRescheduleType.id,
			],
			extendTermForDailyRepayments: [
				this.workingDaysData.extendTermForDailyRepayments,
			],
		});
	}
	...
	setWeeklyWorkingDays() {
		const days = this.workingDaysData.recurrence.replace(
	...
	createRecurrenceFormArray() {
		return this.weekDays.map(
			(weekDay) => new UntypedFormControl(weekDay.checked),
		);
	}
