...
export class CreateLoanProvisioningCriteriaComponent implements OnInit {
	...
	constructor(
		...
		this.route.data.subscribe(
			(data: { loanProvisioningCriteriaTemplate: any }) => {
				this.loanProvisioningCriteriaTemplate =
					data.loanProvisioningCriteriaTemplate;
				this.definitions = this.loanProvisioningCriteriaTemplate.definitions;
				this.liabilityAccounts =
					this.loanProvisioningCriteriaTemplate.glAccounts.filter(
	...
	editDefinition(definition: any) {
		...
		const editDefinitionDialogRef = this.dialog.open(FormDialogComponent, {
			data,
		});
		editDefinitionDialogRef.afterClosed().subscribe((response: any) => {
			if (response.data) {
				...
				this.definitions.splice(
					this.definitions.indexOf(definition),
					1,
					definitionData,
				);
	...
	get provisioningCriteriaFormValid() {
		const definitionsValid: boolean =
			Object.entries(this.isDefinitionValid).length === this.definitions.length
				? true
				: false;
	...
	submit() {
		...
		const loanProvisioningCriteria = {
			...
			loanProducts: this.provisioningCriteriaForm
				.get("loanProducts")
