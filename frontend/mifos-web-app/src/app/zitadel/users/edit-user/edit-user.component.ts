...
export class EditUserComponent implements OnInit {
	...
	constructor(
		...
		this.route.data.subscribe((data: { user: any; usersTemplate: any }) => {
			...
			this.UsersServiceZitadel.getExtraUserData(this.userData.id).subscribe(
				(resp: any) => {
	...
	createEditUserForm() {
		...
		const gender =
			profile?.gender && profile.gender !== "" ? profile.gender : "GENDER_MALE";
		...
		this.editUserForm = this.formBuilder.group({
			...
			staffId: [this.userData.staffId || null],
			roles: [
	...
	submit() {
		...
		this.UsersServiceZitadel.editOffice(officePayload).subscribe(
			(response: any) => {
