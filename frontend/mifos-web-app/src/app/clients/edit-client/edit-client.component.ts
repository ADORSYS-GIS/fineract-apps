...
export class EditClientComponent implements OnInit {
	...
	ngOnInit() {
		...
		this.editClientForm.patchValue({
			...
			legalFormId:
				this.clientDataAndTemplate.legalForm &&
				this.clientDataAndTemplate.legalForm.id,
			accountNo: this.clientDataAndTemplate.accountNo,
			externalId: this.clientDataAndTemplate.externalId,
			genderId:
				this.clientDataAndTemplate.gender &&
				this.clientDataAndTemplate.gender.id,
			...
			dateOfBirth:
				this.clientDataAndTemplate.dateOfBirth &&
				new Date(this.clientDataAndTemplate.dateOfBirth),
			clientTypeId:
				this.clientDataAndTemplate.clientType &&
				this.clientDataAndTemplate.clientType.id,
	...
	setOptions() {
		...
		this.clientClassificationTypeOptions =
			this.clientDataAndTemplate.clientClassificationOptions;
		this.businessLineOptions =
			this.clientDataAndTemplate.clientNonPersonMainBusinessLineOptions;
		this.constitutionOptions =
			this.clientDataAndTemplate.clientNonPersonConstitutionOptions;
	...
	buildDependencies() {
		this.editClientForm
			.get("legalFormId")
			.valueChanges.subscribe((legalFormId: any) => {
				if (legalFormId === 1) {
					...
					this.editClientForm.addControl(
						...
						new UntypedFormControl(
							this.clientDataAndTemplate.firstname,
							Validators.required,
						),
					);
					this.editClientForm.addControl(
					...
					this.editClientForm.addControl(
						"lastname",
						new UntypedFormControl(
							this.clientDataAndTemplate.lastname,
							Validators.required,
						),
					);
				} else {
					...
					this.editClientForm.addControl(
						"fullname",
						new UntypedFormControl(
							this.clientDataAndTemplate.fullname,
							Validators.required,
						),
					);
					this.editClientForm.addControl(
	...
	submit() {
		...
		const clientData = {
			...
			activationDate: this.dateUtils.formatDate(
				editClientFormValue.activationDate,
				dateFormat,
			),
			dateFormat,
			locale,
