...
export class ClientGeneralStepComponent implements OnInit {
	...
	buildDependencies() {
		this.createClientForm
			.get("legalFormId")
			.valueChanges.subscribe((legalFormId: number) => {
		...
		this.createClientForm
			.get("active")
			.valueChanges.subscribe((active: boolean) => {
				if (active) {
					this.createClientForm.addControl(
						"activationDate",
		...
		this.createClientForm
			.get("officeId")
			.valueChanges.subscribe((officeId: number) => {
				this.clientService
