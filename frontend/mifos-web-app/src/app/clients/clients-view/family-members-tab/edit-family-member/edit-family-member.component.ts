...
export class EditFamilyMemberComponent implements OnInit {
	...
	constructor(
		...
		this.route.data.subscribe(
			(data: { clientTemplate: any; editFamilyMember: any }) => {
	...
	ngOnInit() {
		...
		this.editFamilyMemberForm
			.get("dateOfBirth")
			.valueChanges.subscribe((dateOfBirth: any) => {
				if (dateOfBirth) {
	...
	createEditFamilyMemberForm(familyMember: any) {
		this.editFamilyMemberForm = this.formBuilder.group({
			...
			age: [{ value: familyMember.age, disabled: true }],
	...
	submit() {
		...
		if (prevDateOfBirth) {
			if (formValue.dateOfBirth instanceof Date) {
				formValue.dateOfBirth = this.dateUtils.formatDate(
					prevDateOfBirth,
					dateFormat,
				);
			}
		...
		this.clientsService
			.editFamilyMember(
				this.familyMemberDetails.clientId,
				this.familyMemberDetails.id,
				data,
			)
			.subscribe((res) => {
