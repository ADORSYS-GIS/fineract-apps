...
export class ClientsViewComponent implements OnInit {
		...
		this.clientsService
			...
			.subscribe({
				next: (base64Image: any) => {
					...
					if (base64Image) {
						this.clientImage =
							this._sanitizer.bypassSecurityTrustResourceUrl(base64Image);
					} else {
		...
		switch (name) {
			...
			case "Create Standing Instructions":
				...
				this.router.navigate(
					["standing-instructions/create-standing-instructions"],
					{
			...
			case "View Standing Instructions":
				...
				this.router.navigate(
					["standing-instructions/list-standing-instructions"],
					{
						relativeTo: this.route,
	...
	private deleteClient() {
		...
		deleteClientDialogRef.afterClosed().subscribe((response: any) => {
			if (response.delete) {
				this.clientsService
					.deleteClient(this.clientViewData.id)
					.subscribe(() => {
	...
	private unassignStaff() {
		const unAssignStaffDialogRef = this.dialog.open(
	...
	private viewSignature() {
		this.clientsService
			.getClientDocuments(this.clientViewData.id)
			.subscribe((documents: any) => {
				const viewSignatureDialogRef = this.dialog.open(
	...
	private uploadSignature() {
		const uploadSignatureDialogRef = this.dialog.open(
	...
	private deleteSignature() {
		this.clientsService
			.getClientDocuments(this.clientViewData.id)
			.subscribe((documents: any) => {
				const deleteSignatureDialogRef = this.dialog.open(
	...
	private deleteProfileImage() {
		const deleteClientImageDialogRef = this.dialog.open(DeleteDialogComponent, {
			data: {
				deleteContext: `the profile image of ${this.clientViewData.displayName}`,
			},
		});
		deleteClientImageDialogRef.afterClosed().subscribe((response: any) => {
			if (response.delete) {
				this.clientsService
					.deleteClientProfileImage(this.clientViewData.id)
					.subscribe(() => {
						this.reload();
					});
			}
