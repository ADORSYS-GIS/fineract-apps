...
export class AddressTabComponent {
		...
		addAddressDialogRef.afterClosed().subscribe((response: any) => {
			if (response.data) {
				this.clientService
					.createClientAddress(
						this.clientId,
						response.data.value.addressType,
						response.data.value,
					)
					.subscribe((res: any) => {
	...
	editAddress(address: any, index: number) {
		...
		const editAddressDialogRef = this.dialog.open(FormDialogComponent, {
			data,
		});
	...
	isFieldEnabled(fieldName: any) {
		return this.clientAddressFieldConfig.find(
			(fieldObj: any) => fieldObj.field === fieldName,
		)?.isEnabled;
	}
	...
	getSelectedValue(fieldName: any, fieldId: any) {
		return this.clientAddressTemplate[fieldName].find(
	...
	getAddressFormFields(formType?: string, address?: any) {
		...
		for (
			let index = 0;
			index < this.clientAddressTemplate.addressTypeIdOptions.length;
			index++
		) {
		...
		formfields.push(
			this.isFieldEnabled("addressLine1")
				? new InputBase({
						...
						label:
							this.translateService.instant("labels.inputs.Address Line") +
							" 1",
