...
export class AddClientCollateralComponent implements OnInit {
	 ...
	 */
	buildDependencies() {
		this.clientCollateralForm.controls.collateralId.valueChanges.subscribe(
			(collateralId) => {
		...
		this.clientCollateralForm.controls.quantity.valueChanges.subscribe(
			(quantity: any) => {
				this.clientCollateralForm.patchValue({
					totalValue: this.collateralDetails.basePrice * quantity,
					totalCollateralValue:
						(this.collateralDetails.basePrice *
							this.collateralDetails.pctToBase *
							quantity) /
						100,
				});
			},
