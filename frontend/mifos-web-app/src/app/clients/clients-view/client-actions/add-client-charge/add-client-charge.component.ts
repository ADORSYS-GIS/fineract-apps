...
export class AddClientChargeComponent implements OnInit {
	 ...
	 */
	buildDependencies() {
		this.clientChargeForm.controls.chargeId.valueChanges.subscribe(
			(chargeId) => {
				this.clientsService
		...
		if (this.chargeDetails.dueDateNotRequired !== true) {
			if (this.chargeDetails.chargeTimeTypeAnnualOrMonth === true) {
				...
				if (clientCharge.feeOnMonthDay) {
					...
					clientCharge.feeOnMonthDay = this.dateUtils.formatDate(
						prevDate,
						monthDayFormat,
					);
				}
				...
				if (clientCharge.dueDate) {
					...
					clientCharge.dueDate = this.dateUtils.formatDate(
						prevDate,
						dateFormat,
					);
				}
