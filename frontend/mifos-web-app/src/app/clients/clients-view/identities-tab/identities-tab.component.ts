...
export class IdentitiesTabComponent {
	...
	/** Identifiers Table */
	@ViewChild("identifiersTable", { static: true })
	identifiersTable: MatTable<Element>;
	...
	constructor(
		...
		this.route.data.subscribe(
			(data: { clientIdentities: any; clientIdentifierTemplate: any }) => {
	...
	addIdentifier() {
		...
		const formfields: FormfieldBase[] = [
			...
			new SelectBase({
				...
				options: {
					...
					data: [
						{
							label: this.translateService.instant("labels.catalogs.Active"),
							value: "Active",
						},
						{
							label: this.translateService.instant("labels.catalogs.Inactive"),
							value: "Inactive",
						},
					],
				},
		...
		const data = {
			title: this.translateService.instant(
				"labels.heading.Add Client Identifier",
			),
			formfields: formfields,
		};
		const addIdentifierDialogRef = this.dialog.open(FormDialogComponent, {
			data,
		});
				...
				this.clientService
					...
					.subscribe((res: any) => {
						this.clientIdentities.push({
							...
							documentType:
								this.clientIdentifierTemplate.allowedDocumentTypes.filter(
	...
	uploadDocument(index: number, identifierId: string) {
		const uploadDocumentDialogRef = this.dialog.open(
