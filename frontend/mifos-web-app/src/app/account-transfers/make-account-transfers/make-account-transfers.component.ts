...
export class MakeAccountTransfersComponent implements OnInit, AfterViewInit {
	...
	setParams() {
		...
		switch (this.accountType) {
			...
			case "interbank":
				...
				this.interbank =
					this.route.snapshot.queryParams["interbank"] === "true";
	...
	setOptions() {
		...
		this.toAccountTypeData =
			this.accountTransferTemplateData.toAccountTypeOptions;
	...
	ngAfterViewInit() {
		if (!this.interbank) {
			this.makeAccountTransferForm.controls.toClientId.valueChanges.subscribe(
				(value: string) => {
					if (value.length >= 2) {
						this.clientsService
	...
	makeTransfer() {
		...
		this.accountTransfersService
			.createAccountTransfer(makeAccountTransferData)
			.subscribe(() => {
				this.isLoading = false;
	...
	makeInterbankTransfer() {
		...
		this.accountTransfersService
			.sendInterbankTransfer(JSON.stringify(payload))
			.subscribe(
				(trnsfr) => {
	...
	searchAccountByNumber() {
		...
		this.accountTransfersService
			.getAccountByNumber(
				this.phoneAccount,
				this.accountTransferTemplateData.currency.code,
			)
			.subscribe(
				(acc) => {
