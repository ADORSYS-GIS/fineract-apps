...
export class ListStandingInstructionsComponent {
	...
	@ViewChild("instructionsTable", { static: true })
	instructionTableRef: MatTable<Element>;
	...
	constructor(
		...
		this.route.data.subscribe((data: { standingIntructionsTemplate: any }) => {
			...
			if (data.standingIntructionsTemplate.fromClient) {
				this.clientName =
					this.standingIntructionsTemplateData.fromClient.displayName;
				this.getStandingInstructions();
			}
			this.setParams();
			this.transferTypeDatas =
				this.standingIntructionsTemplateData.transferTypeOptions;
		});
	}
	...
	setParams() {
		...
		this.isFromClient = this.route.parent.parent.snapshot.params["clientId"]
			? true
			: false;
	}
	...
	getStandingInstructions() {
		...
		const searchData = {
			clientId:
				this.standingIntructionsTemplateData.fromClient.id ||
				this.fromClientId.value,
			clientName:
				this.standingIntructionsTemplateData.fromClient.displayName ||
				this.clientNameControl.value,
			locale,
			dateFormat,
			limit: 14,
			offset: 0,
		...
		this.accountTransfersService
			.getStandingInstructions(searchData)
	...
	deleteStandingInstruction(instructionId: any) {
		const deleteStandingInstructionDialogRef = this.dialog.open(
