...
export class RunReportComponent implements OnInit {
		...
		this.route.data.subscribe(
			(data: { reportParameters: ReportParameter[]; configurations: any }) => {
				...
				if (this.isTableReport()) {
					...
					const reportExportS3Config =
						data.configurations.globalConfiguration.find(
							(config: GlobalConfiguration) =>
								config.name === "report-export-s3-folder-name",
						);
	...
	updateParentParameters(parent: ReportParameter) {
		const parentNames = this.parentParameters.map(
			...
			this.reportForm.get(param.name).valueChanges.subscribe((option: any) => {
				param.childParameters.forEach((child: ReportParameter) => {
					if (child.displayType === "none") {
						this.reportForm.addControl(
							child.name,
							new UntypedFormControl(child.defaultVal),
						);
					} else {
						this.reportForm.addControl(
							child.name,
