...
export class WebAppComponent implements OnInit, OnDestroy {
	...
	ngOnInit() {
		...
		if (localStorage.getItem("mifosXLocation")) {
			const activitiesArray: string[] = JSON.parse(
				localStorage.getItem("mifosXLocation"),
			);
			const length = activitiesArray.length;
			activities =
				length > 100 ? activitiesArray.slice(length - 100) : activitiesArray;
		}
		...
		if (!localStorage.getItem("mifosXTenantIdentifier")) {
			this.settingsService.setTenantIdentifier(
				environment.fineractPlatformTenantId || "default",
			);
		}
		this.settingsService.setTenantIdentifiers(
			environment.fineractPlatformTenantIds.split(","),
		);
		...
		if (!(routeD === undefined)) {
			switch (routeD.id) {
				...
				case "submit":
					...
					break;
				default:
					this.router.navigate([routeD.route], {
