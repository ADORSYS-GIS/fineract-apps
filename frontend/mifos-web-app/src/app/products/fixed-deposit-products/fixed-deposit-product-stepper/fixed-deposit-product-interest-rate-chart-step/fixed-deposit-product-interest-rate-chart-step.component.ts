...
@Component({
	...
	templateUrl:
		"./fixed-deposit-product-interest-rate-chart-step.component.html",
	styleUrls: [
		"./fixed-deposit-product-interest-rate-chart-step.component.scss",
	],
	animations: [
...
export class FixedDepositProductInterestRateChartStepComponent
	implements OnInit
{
	...
	ngOnInit() {
		this.periodTypeData =
			this.fixedDepositProductsTemplate.chartTemplate.periodTypes;
		this.entityTypeData =
			this.fixedDepositProductsTemplate.chartTemplate.entityTypeOptions;
		this.attributeNameData =
			this.fixedDepositProductsTemplate.chartTemplate.attributeNameOptions;
		this.conditionTypeData =
			this.fixedDepositProductsTemplate.chartTemplate.conditionTypeOptions;
		this.genderData =
			this.fixedDepositProductsTemplate.chartTemplate.genderOptions;
		this.clientTypeData =
			this.fixedDepositProductsTemplate.chartTemplate.clientTypeOptions;
		...
		this.incentiveTypeData =
			this.fixedDepositProductsTemplate.chartTemplate.incentiveTypeOptions;
	...
	assignFormData() {
		this.addChart();
		const isChartArray = Array.isArray(
			this.fixedDepositProductsTemplate.activeChart,
		);
		if (this.fixedDepositProductsTemplate.activeChart) {
			if (!isChartArray) {
				this.chartDetailData.push(
					this.fixedDepositProductsTemplate.activeChart,
				);
			} else {
		...
		this.charts.controls.forEach(
			(chartDetailControl: UntypedFormGroup, i: number) => {
				if (!this.chartsDetail[i]) {
					return;
				}

				// Iterate for every chartSlab in chart
				this.chartsDetail[i].chartSlabs.forEach(
					(chartSlabDetail: any, j: number) => {
						...
						const formArray = chartDetailControl.controls[
							"chartSlabs"
						] as UntypedFormArray;
						...
						this.chartsDetail[i].chartSlabs[j].incentives.forEach(
							(chartIncentiveDetail: any) => {
								...
								const newFormArray = (chartIncentiveControl as UntypedFormGroup)
									.controls["incentives"] as UntypedFormArray;
	...
	get charts(): UntypedFormArray {
		return this.fixedDepositProductInterestRateChartForm.get(
			"charts",
		) as UntypedFormArray;
	}
	...
	getIncentives(
		chartSlabs: UntypedFormArray,
		chartSlabIndex: number,
	): UntypedFormArray {
	...
	editChartSlab(chartSlabs: UntypedFormArray, chartSlabIndex: number) {
		const data = {
			...
			layout: {
				addButtonText: this.translateService.instant("labels.text.this"),
			},
		};
		const dialogRef = this.dialog.open(FormDialogComponent, { data });
		dialogRef.afterClosed().subscribe((response: any) => {
	...
	delete(formArray: UntypedFormArray, index: number) {
		const dialogRef = this.dialog.open(DeleteDialogComponent, {
			data: {
				deleteContext: this.translateService.instant("labels.text.this"),
			},
		});
		dialogRef.afterClosed().subscribe((response: any) => {
			if (response.delete) {
	...
	getData(formType: string, values?: any) {
		switch (formType) {
			...
			case "Incentive":
				return {
					values,
					chartTemplate: this.fixedDepositProductsTemplate.chartTemplate,
				};
		}
	}
	...
	get fixedDepositProductInterestRateChart() {
		...
		for (const chart of fixedDepositProductInterestRateChart.charts) {
			...
			if (chart.fromDate instanceof Date) {
				chart.fromDate = this.dateUtils.formatDateAsString(
					chart.fromDate,
					dateFormat,
				);
			}
			if (chart.endDate) {
				if (chart.endDate instanceof Date) {
					chart.endDate = this.dateUtils.formatDateAsString(
						chart.endDate,
						dateFormat,
					);
				}
