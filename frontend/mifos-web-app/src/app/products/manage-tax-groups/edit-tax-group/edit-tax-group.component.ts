...
export class EditTaxGroupComponent implements OnInit {
	...
	addTaxGroup() {
		...
		const taxComponentDialogRef = this.dialog.open(FormDialogComponent, {
			data,
		});
	...
	editTaxGroup(taxComponent: any, index: any) {
		const formfields: FormfieldBase[] = [
			...
			new DatepickerBase({
				controlName: "startDate",
				value: taxComponent.startDate
					? new Date(taxComponent.startDate)
					: new Date(),
				label: "Start Date",
				minDate: this.minDate,
				maxDate: this.maxDate,
	...
	assignFormData() {
		this.taxGroupData.taxAssociations.forEach((taxComponentData: any) => {
			const chart = {
				...
				startDate: taxComponentData.startDate
					? new Date(taxComponentData.startDate)
					: "",
				endDate: taxComponentData.endDate
					? new Date(taxComponentData.endDate)
					: "",
	...
	submit() {
		...
		for (const taxComponent of taxGroup.taxComponents) {
			taxComponent.startDate =
				this.dateUtils.formatDate(taxComponent.startDate, dateFormat) || "";
			if (taxComponent.endDate) {
				delete taxComponent.startDate;
				taxComponent.endDate =
					this.dateUtils.formatDate(taxComponent.endDate, dateFormat) || "";
			}
