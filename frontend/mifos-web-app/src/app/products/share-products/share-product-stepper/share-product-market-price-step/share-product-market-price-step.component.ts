...
export class ShareProductMarketPriceStepComponent implements OnInit {
	...
	get marketPricePeriods(): UntypedFormArray {
		return this.shareProductMarketPriceForm.get(
			"marketPricePeriods",
		) as UntypedFormArray;
	}
	...
	addMarketPricePeriod() {
		...
		const addMarketPricePeriodDialogRef = this.dialog.open(
			FormDialogComponent,
			{ data },
		);
	...
	deleteMarketPricePeriod(index: number) {
		const deleteMarketPricePeriodDialogRef = this.dialog.open(
		...
		const formfields: FormfieldBase[] = [
			...
			new InputBase({
				...
				label: this.translateService.instant(
					"labels.inputs.Nominal/Unit Price",
				),
