...
export class ShareProductAccountingStepComponent implements OnInit {
	...
	ngOnInit() {
		...
		this.incomeAccountData =
			this.shareProductsTemplate.accountingMappingOptions
		...
		if (this.shareProductsTemplate.accountingRule) {
			...
			if (this.shareProductsTemplate.accountingRule.id === 2) {
				this.shareProductAccountingForm.patchValue({
					shareReferenceId:
						this.shareProductsTemplate.accountingMappings.shareReferenceId.id,
					shareSuspenseId:
						this.shareProductsTemplate.accountingMappings.shareSuspenseId.id,
					shareEquityId:
						this.shareProductsTemplate.accountingMappings.shareEquityId.id,
					incomeFromFeeAccountId:
						this.shareProductsTemplate.accountingMappings.incomeFromFeeAccountId
							.id,
				});
			}
		}
		...
		this.shareProductAccountingForm
			...
			.valueChanges.subscribe((accountingRule: any) => {
				if (accountingRule === 2) {
					...
					this.shareProductAccountingForm.removeControl(
						"incomeFromFeeAccountId",
					);
				}
			});
