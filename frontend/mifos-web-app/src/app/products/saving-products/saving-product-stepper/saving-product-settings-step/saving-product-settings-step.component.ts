	...
	ngOnInit() {
		...
		this.savingProductSettingsForm.patchValue({
			...
			withdrawalFeeForTransfers:
				this.savingProductsTemplate.withdrawalFeeForTransfers,
			minBalanceForInterestCalculation:
				this.savingProductsTemplate.minBalanceForInterestCalculation,
			enforceMinRequiredBalance:
				this.savingProductsTemplate.enforceMinRequiredBalance,
			...
			taxGroupId:
				this.savingProductsTemplate.taxGroup &&
				this.savingProductsTemplate.taxGroup.id,
			isDormancyTrackingActive:
				this.savingProductsTemplate.isDormancyTrackingActive,
		...
		this.savingProductSettingsForm
			...
			.valueChanges.subscribe((allowOverdraft: any) => {
				if (allowOverdraft) {
					...
					this.savingProductSettingsForm.removeControl(
						"minOverdraftForInterestCalculation",
					);
					this.savingProductSettingsForm.removeControl(
						"nominalAnnualInterestRateOverdraft",
