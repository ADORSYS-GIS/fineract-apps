...
export class RecurringDepositProductPreviewStepComponent
	implements OnInit, OnChanges
{
	...
	setCurrentValues(): void {
		if (this.isCashOrAccrualAccounting()) {
			...
			if (this.isAccrualAccounting()) {
				this.accountingMappings["feeReceivableAccount"] =
					this.accounting.glAccountLookUp(
						this.recurringDepositProduct.feesReceivableAccountId,
						assetAccountData,
					);
				this.accountingMappings["penaltyReceivableAccount"] =
					this.accounting.glAccountLookUp(
	...
	isNoneAccounting(): boolean {
		return this.accounting.isNoneAccountingRuleId(
			this.recurringDepositProduct.accountingRule,
		);
	}
