...
@Component({
	...
	imports: [...STANDALONE_SHARED_IMPORTS, GlAccountSelectorComponent],
})
export class CreateTaxComponentComponent implements OnInit {
	...
	setConditionalControls() {
		this.taxComponentForm
			...
			.valueChanges.subscribe((debitAccountTypeId) => {
		...
		this.taxComponentForm
			...
			.valueChanges.subscribe((creditAccountTypeId) => {
	...
	getAccountsData(accountTypeId: number) {
		switch (accountTypeId) {
			case 1:
				return (
					this.taxComponentTemplateData.glAccountOptions.assetAccountOptions ||
					[]
				);
			case 2:
				return (
					this.taxComponentTemplateData.glAccountOptions
						.liabilityAccountOptions || []
				);
			case 3:
				return (
					this.taxComponentTemplateData.glAccountOptions.equityAccountOptions ||
					[]
				);
			case 4:
				return (
					this.taxComponentTemplateData.glAccountOptions.incomeAccountOptions ||
					[]
				);
			case 5:
				return (
	...
	submit() {
		...
		if (taxComponentFormData.startDate instanceof Date) {
			taxComponentFormData.startDate = this.dateUtils.formatDate(
				prevStartDate,
				dateFormat,
			);
		}
