...
export class CreateChargeComponent implements OnInit {
	...
	constructor(
		...
		this.route.data.subscribe((data: { chargesTemplate: any }) => {
			...
			if (
				data.chargesTemplate.incomeOrLiabilityAccountOptions
	...
	createChargeForm() {
		this.chargeForm = this.formBuilder.group({
			...
			penalty: [false],
			taxGroupId: [null],
			minCap: [null, [maxNumberValueValidator("maxCap")]],
			maxCap: [null, [minNumberValueValidator("minCap")]],
		});
	}
	...
	filteredChargeCalculationType(): any {
		return this.chargeCalculationTypeData.filter(
	...
	showMinMaxCap(): boolean {
		...
		const chargeCalculationType =
			this.chargeForm.controls.chargeCalculationType.value;
		...
		if (chargeAppliesTo === 1) {
			...
			return (
				(chargeTimeType === 16 || chargeTimeType === 5) &&
				chargeCalculationType === 2
			);
		} else if (chargeAppliesTo === 4) {
			return (
				(chargeTimeType === 14 || chargeTimeType === 15) &&
				chargeCalculationType === 2
			);
		}
		return false;
	...
	setConditionalControls() {
		...
		this.chargeForm
			.get("chargeTimeType")
			.valueChanges.subscribe((chargeTimeType) => {
				...
				switch (chargeTimeType) {
					...
					case 9: // Overdue Fee
						...
						this.chargeForm
							.get("addFeeFrequency")
							.valueChanges.subscribe((addFeeFrequency) => {
								if (addFeeFrequency) {
									this.chargeForm.addControl(
										"feeFrequency",
										new UntypedFormControl("", Validators.required),
		...
		this.chargeForm
			...
			.valueChanges.subscribe((currencyCode) => {
				this.currencyDecimalPlaces =
					this.chargesTemplateData.currencyOptions.find(
	...
	submit() {
		...
		if (chargeFormData.feeOnMonthDay instanceof Date) {
			chargeFormData.feeOnMonthDay = this.dateUtils.formatDate(
				prevFeeOnMonthDay,
				monthDayFormat,
			);
		}
