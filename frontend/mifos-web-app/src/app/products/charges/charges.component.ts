...
export class ChargesComponent implements OnInit, AfterViewInit {
	...
	@ViewChild("templateButtonCreateCharge")
	templateButtonCreateCharge: TemplateRef<any>;
	...
	ngAfterViewInit() {
		...
		if (this.configurationWizardService.showChargesList === true) {
			setTimeout(() => {
				this.showPopover(
					this.templateChargesTable,
					this.chargesTable.nativeElement,
					"top",
					true,
				);
			});
	...
	filterByAppliesTo(chargeAppliesTo: number) {
		const filteredCharges: Charge[] = this.chargeData.filter(
			(charge: Charge) => {
