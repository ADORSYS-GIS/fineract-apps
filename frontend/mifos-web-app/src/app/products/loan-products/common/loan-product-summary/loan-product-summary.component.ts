...
export class LoanProductSummaryComponent implements OnInit, OnChanges {
	...
	setCurrentValues(): void {
		this.isAdvancedPaymentAllocation =
		...
		if (this.action === "view") {
			...
			this.feeToIncomeAccountMappings =
				this.loanProduct.feeToIncomeAccountMappings || [];
			this.penaltyToIncomeAccountMappings =
				this.loanProduct.penaltyToIncomeAccountMappings || [];
			...
			this.writeOffReasonsToExpenseMappings =
				this.loanProduct.writeOffReasonsToExpenseMappings || [];
		} else {
			this.accountingMappings = {};

			if (
				(this.loanProduct.accountingRule &&
					this.loanProduct.accountingRule > 1) ||
				this.loanProductsTemplate.accountingRule.value !== "NONE"
			) {
				const assetAccountData =
					this.loanProductsTemplate.accountingMappingOptions
						.assetAccountOptions || [];
				...
				const assetAndLiabilityAccountData =
					this.loanProductsTemplate.accountingMappingOptions
						.assetAndLiabilityAccountOptions || [];
				const chargeOffReasonOptions: any =
					this.loanProductsTemplate.chargeOffReasonOptions || [];
				const writeOffReasonOptions: any =
					this.loanProductsTemplate.writeOffReasonOptions || [];
				const buydownFeeClassificationOptions: any =
					this.loanProductsTemplate.buydownFeeClassificationOptions || [];
				...
				if (this.loanProduct.paymentChannelToFundSourceMappings?.length > 0) {
					const paymentTypesData =
						this.loanProductsTemplate.paymentTypeOptions || [];
					this.loanProduct.paymentChannelToFundSourceMappings.forEach(
						(m: any) => {
							this.paymentChannelToFundSourceMappings.push({
								...
								paymentType: this.paymentTypeLookUp(
									m.paymentTypeId,
									paymentTypesData,
								),
							});
				...
				if (this.loanProduct.feeToIncomeAccountMappings?.length > 0) {
					this.loanProduct.feeToIncomeAccountMappings.forEach((m: any) => {
						this.feeToIncomeAccountMappings.push({
							incomeAccount: this.glAccountLookUp(
								m.incomeAccountId,
								incomeAccountData,
							),
							charge: this.chargeLookUp(
								m.chargeId,
								this.loanProductsTemplate.chargeOptions,
							),
						});
					});
				}
				...
				if (this.loanProduct.penaltyToIncomeAccountMappings?.length > 0) {
					this.loanProduct.penaltyToIncomeAccountMappings.forEach((m: any) => {
						this.penaltyToIncomeAccountMappings.push({
							...
							charge: this.chargeLookUp(
								m.chargeId,
								this.loanProductsTemplate.penaltyOptions,
							),
						});
					});
				}
					...
					this.loanProduct.chargeOffReasonToExpenseAccountMappings.forEach(
						(m: ChargeOffReasonToExpenseAccountMapping) => {
							...
							this.chargeOffReasonToExpenseAccountMappings.push({
								expenseAccount: this.glAccountLookUp(
									m.expenseAccountId,
									expenseAccountData,
								),
								reasonCodeValue: {
				...
				if (
					...
					this.loanProduct.buydownfeeClassificationToIncomeAccountMappings.forEach(
						(m: any) => {
							let optionData = this.optionDataLookUp(
				...
				if (
					this.loanProduct
						.capitalizedIncomeClassificationToIncomeAccountMappings?.length > 0
				) {
					this.loanProduct.capitalizedIncomeClassificationToIncomeAccountMappings.forEach(
						(m: any) => {
							let optionData = this.optionDataLookUp(
								m.classificationCodeValueId,
				...
				if (this.loanProduct.writeOffReasonsToExpenseMappings?.length > 0) {
					this.loanProduct.writeOffReasonsToExpenseMappings.forEach(
						(m: any) => {
							let optionData = this.optionDataLookUp(
								m.writeOffReasonCodeValueId,
			...
			if (this.loanProduct.isInterestRecalculationEnabled) {
				this.loanProduct.interestRecalculationData = {
					...
					isArrearsBasedOnOriginalSchedule:
						this.loanProduct.isArrearsBasedOnOriginalSchedule,
					isCompoundingToBePostedAsTransaction:
						this.loanProduct.isCompoundingToBePostedAsTransaction,
					recalculationRestFrequencyInterval:
						this.loanProduct.recalculationRestFrequencyInterval,
			...
			if (
				this.isAdvancedPaymentAllocation &&
				this.loanProduct.daysInYearType?.id == 1
			) {
				optionValue = this.optionDataLookUp(
			...
			if (
				this.isAdvancedPaymentAllocation &&
				this.loanProduct.enableIncomeCapitalization
			) {
				optionValue = this.optionDataLookUp(
			...
			if (
				this.isAdvancedPaymentAllocation &&
				this.loanProduct.enableBuyDownFee
			) {
				optionValue = this.optionDataLookUp(
			...
			if (
				...
				!this.loanProduct.loanScheduleType.value
			) {
			...
			delinquencyBuckets.some((delinquencyBucket: DelinquencyBucket) => {
				if (delinquencyBucket.id === delinquencyBucketId) {
					delinquencyBucketData = {
						id: delinquencyBucket.id,
						name: delinquencyBucket.name,
					};
				}
			});
		}
	...
	isAdvancedAccountingEnabled(): boolean {
		return (
			...
			this.loanProduct.chargeOffReasonToExpenseAccountMappings?.length > 0 ||
			this.loanProduct.buydownfeeClassificationToIncomeAccountMappings?.length >
				0 ||
