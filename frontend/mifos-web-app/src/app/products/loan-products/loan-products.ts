...
export class LoanProducts {
	...
	public setItemsByDefault(configurations: any) {
		...
		configurations.globalConfiguration.forEach(
	...
	public updateLoanProductDefaults(
		loanProductTemplate: any,
		isForUpdate: boolean,
	) {
		const itemsByDefault: GlobalConfiguration[] =
			loanProductTemplate["itemsByDefault"];
		itemsByDefault.forEach((config: GlobalConfiguration) => {
			const propertyName = this.resolvePropertyName(config.name);
			if (propertyName !== "") {
				if (isForUpdate) {
					if (
						!loanProductTemplate[propertyName] ||
						loanProductTemplate[propertyName] === ""
					) {
