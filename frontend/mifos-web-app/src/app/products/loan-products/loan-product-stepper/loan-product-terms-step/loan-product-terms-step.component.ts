...
export class LoanProductTermsStepComponent implements OnInit, OnChanges {
	...
	ngOnInit() {
		this.valueConditionTypeData =
			this.loanProductsTemplate.valueConditionTypeOptions;
		...
		this.repaymentFrequencyTypeData =
			this.loanProductsTemplate.repaymentFrequencyTypeOptions;
		this.repaymentStartDateTypeOptions =
		...
		this.loanProductTermsForm.patchValue({
			...
			isLinkedToFloatingInterestRates:
				this.loanProductsTemplate.isLinkedToFloatingInterestRates,
			minInterestRatePerPeriod:
				this.loanProductsTemplate.minInterestRatePerPeriod,
			interestRatePerPeriod: this.loanProductsTemplate.interestRatePerPeriod,
			maxInterestRatePerPeriod:
				this.loanProductsTemplate.maxInterestRatePerPeriod,
			interestRateFrequencyType:
				this.loanProductsTemplate.interestRateFrequencyType.id,
			floatingRatesId: this.loanProductsTemplate.floatingRateId,
			interestRateDifferential:
				this.loanProductsTemplate.interestRateDifferential,
			...
			minDifferentialLendingRate:
				this.loanProductsTemplate.minDifferentialLendingRate,
			defaultDifferentialLendingRate:
				this.loanProductsTemplate.defaultDifferentialLendingRate,
			maxDifferentialLendingRate:
				this.loanProductsTemplate.maxDifferentialLendingRate,
			...
			repaymentStartDateType:
				this.loanProductsTemplate.repaymentStartDateType.id || 1,
			interestRecognitionOnDisbursementDate:
				this.loanProductsTemplate.interestRecognitionOnDisbursementDate ||
				false,
		});
		...
		this.loanProductTermsForm.setControl(
			...
			this.formBuilder.array(
				this.loanProductsTemplate.principalVariationsForBorrowerCycle.map(
					(variation: any) => ({
						...variation,
		...
		this.loanProductTermsForm.setControl(
			...
			this.formBuilder.array(
				this.loanProductsTemplate.interestRateVariationsForBorrowerCycle.map(
					(variation: any) => ({
						...variation,
						valueConditionType: variation.valueConditionType.id,
					}),
	...
	setConditionalControls() {
		this.loanProductTermsForm
			...
			.valueChanges.subscribe((allowApprovedDisbursedAmountsOverApplied) => {
				if (allowApprovedDisbursedAmountsOverApplied) {
					...
					this.loanProductTermsForm.removeControl(
						"disallowExpectedDisbursements",
					);
				}
			});
			...
			.valueChanges.subscribe((isLinkedToFloatingInterestRates) => {
				if (isLinkedToFloatingInterestRates) {
					...
					this.loanProductTermsForm.addControl(
						"interestRateFrequencyType",
						new UntypedFormControl(
							this.interestRateFrequencyTypeData.id,
							Validators.required,
						),
					);
		...
		this.loanProductTermsForm
			...
			.valueChanges.subscribe((useBorrowerCycle) => {
				if (useBorrowerCycle) {
					this.loanProductTermsForm.addControl(
						"principalVariationsForBorrowerCycle",
						this.formBuilder.array([]),
	...
	addVariationsForBorrowerCycle(
		...
		const addVariationsForBorrowerCycleDialogRef = this.dialog.open(
			FormDialogComponent,
			{ data },
		);
		addVariationsForBorrowerCycleDialogRef
			.afterClosed()
	...
	editVariationsForBorrowerCycle(
		...
		const data = {
			...this.getData(
				formType,
				variationsForBorrowerCycleFormArray.at(index).value,
			),
		...
		addVariationsForBorrowerCycleDialogRef
			...
			.subscribe((response: any) => {
				if (response.data) {
					variationsForBorrowerCycleFormArray
						.at(index)
						.patchValue(response.data.value);
					this.setLoanProductTermsFormDirty();
				}
			});
	}
	...
	deleteVariationsForBorrowerCycle(
		...
		const deleteVariationsForBorrowerCycleDialogRef = this.dialog.open(
			DeleteDialogComponent,
			{
				data: { deleteContext: `this` },
			},
		);
		deleteVariationsForBorrowerCycleDialogRef
			.afterClosed()
			.subscribe((response: any) => {
				if (response.delete) {
	...
	getData(formType: string, values?: any) {
		switch (formType) {
			case "Principal":
				return {
					title: this.translateService.instant(
						"labels.heading.Principal by loan cycle",
					),
			...
			case "NumberOfRepayments":
				return {
					title: this.translateService.instant(
						"labels.heading.Number of Repayments by loan cycle",
					),
					formfields: this.getFormfields(values),
				};
	...
	getFormfields(values?: any) {
		const formfields: FormfieldBase[] = [
			new SelectBase({
				...
				value: values
					? values.valueConditionType
					: this.valueConditionTypeData[0].id,
				options: {
					label: "value",
					value: "id",
					data: this.valueConditionTypeData,
				},
				required: true,
				order: 1,
			}),
	...
	allowFixedLength(): boolean {
		return (
			this.isAdvancedTransactionProcessingStrategy && this.isZeroInterest()
		);
	}
