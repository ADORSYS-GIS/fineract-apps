...
export class AdvancedPaymentStrategy {
	public buildAdvancedCreditAllocationList(
		loanProduct: any,
	...
	public buildAdvancedPaymentAllocationList(
		loanProduct: any,
	...
	public buildPaymentAllocations(
		...
		advancedPaymentAllocations.forEach(
			(paymentAllocation: AdvancedPaymentAllocation) => {
				if (paymentAllocation.paymentAllocationOrder) {
					paymentAllocations.push({
						...
						futureInstallmentAllocationRule:
							paymentAllocation.futureInstallmentAllocationRule.code,
					});
				}
			},
		);
	...
	public buildCreditAllocations(
		...
		advancedCreditAllocations.forEach(
			(creditAllocation: AdvancedCreditAllocation) => {
	...
	private buildCurrentPaymentAllocationOrder(
		...
		currentItems.forEach((item: any) => {
			paymentAllocationOrder.push(
				this.getItemByCode(item.paymentAllocationRule, defaultItems),
			);
		});
		return paymentAllocationOrder;
	}
	...
	private buildCurrentCreditAllocationOrder(
		...
		currentItems.forEach((item: any) => {
			creditAllocationOrder.push(
				this.getItemByCode(item.creditAllocationRule, defaultItems),
			);
		});
	...
	public isDefault(transaction: PaymentAllocationTransactionType): boolean {
		return (
			transaction.code ===
			PaymentAllocationTransactionTypes.DEFAULT_TRANSACTION.code
		);
	}
}
