...
export class LoanProductPaymentStrategyStepComponent implements OnInit {
	...
	@Input()
	advancedPaymentAllocationTransactionTypes: PaymentAllocationTransactionType[] =
		[];
	@Input() paymentAllocationOrderDefault: PaymentAllocationOrder[];
	@Input()
	advancedCreditAllocationTransactionTypes: PaymentAllocationTransactionType[] =
		[];
	...
	sendAllocations(): void {
		this.setPaymentAllocation.emit(
			this.advancedPaymentStrategy.buildPaymentAllocations(
	...
	addTransaction(): void {
		...
		this.advancedPaymentAllocations.forEach(
			(item: AdvancedPaymentAllocation) => {
				transactionTypesCurrent.push(item.transaction.code);
			},
		);
		this.advancedCreditAllocations.forEach(
			(item: AdvancedPaymentAllocation) => {
				transactionTypesCurrent.push(item.transaction.code);
			},
		);
		...
		this.advancedPaymentAllocationTransactionTypes.forEach(
			(option: PaymentAllocationTransactionType) => {
				if (
					...
					option.credit = false;
					option.value = this.translateService.instant(
						"labels.catalogs." + option.value,
					);
		...
		const transactionTypeDialogRef = this.dialog.open(FormDialogComponent, {
			data,
		});
		transactionTypeDialogRef.afterClosed().subscribe((response: any) => {
			if (response.data) {
				...
				transactionTypesOptions.forEach(
					(transactionType: PaymentAllocationTransactionType) => {
	...
	transactionTypeRemoved(transaction: PaymentAllocationTransactionType): void {
		if (!transaction.credit) {
			this.advancedPaymentAllocations.forEach(
				(item: AdvancedPaymentAllocation, index: number) => {
