...
} from "app/shared/models/option-data.model";
...
export class LoanProductSettingsStepComponent implements OnInit {
	...
	DAYS_AFTER_REPAYMENT_IS_OVERDUE =
	...
	ngOnInit() {
		...
		this.rescheduleStrategyTypeData =
			this.loanProductsTemplate.rescheduleStrategyTypeOptions;
		this.rescheduleStrategyTypeDataBase =
			this.loanProductsTemplate.rescheduleStrategyTypeOptions;
		...
		this.interestRecalculationOnDayTypeData = Array.from(
			{ length: 28 },
			(_, index) => index + 1,
		);
		this.delinquencyBucketData =
		...
		this.loanProductSettingsForm.patchValue({
			...
			graceOnPrincipalPayment:
				this.loanProductsTemplate.graceOnPrincipalPayment,
			...
			canDefineInstallmentAmount:
				this.loanProductsTemplate.canDefineInstallmentAmount,
			...
			isInterestRecalculationEnabled:
				this.loanProductsTemplate.isInterestRecalculationEnabled,
			...
			allowAccrualPostingInArrears:
				this.loanProductsTemplate.allowAccrualPostingInArrears,
		...
		this.processingStrategyService.initialize(
			this.isAdvancedTransactionProcessingStrategy,
		);
		...
		if (
			...
			this.loanProductSettingsForm.patchValue({
				...
				overDueDaysForRepaymentEvent:
					this.loanProductsTemplate.overDueDaysForRepaymentEvent,
			});
		} else {
			this.loanProductSettingsForm.patchValue({
		...
		if (this.loanProductsTemplate.isInterestRecalculationEnabled) {
			this.loanProductSettingsForm.patchValue({
				...
				rescheduleStrategyMethod:
					this.loanProductsTemplate.interestRecalculationData
						.rescheduleStrategyType.id,
				interestRecalculationCompoundingMethod:
					this.loanProductsTemplate.interestRecalculationData
						.interestRecalculationCompoundingType.id,
				recalculationRestFrequencyType:
					this.loanProductsTemplate.interestRecalculationData
						.recalculationRestFrequencyType.id,
				...
				recalculationCompoundingFrequencyType:
					this.loanProductsTemplate.interestRecalculationData
						.interestRecalculationCompoundingType.id &&
					this.loanProductsTemplate.interestRecalculationData
						.recalculationCompoundingFrequencyType.id,
				...
				recalculationRestFrequencyNthDayType:
					this.loanProductsTemplate.interestRecalculationData
						.recalculationRestFrequencyType.id === 4 &&
					this.loanProductsTemplate.interestRecalculationData
						...
						: this.loanProductsTemplate.interestRecalculationData
							...
							this.loanProductsTemplate.interestRecalculationData
								.recalculationRestFrequencyNthDay.id,
				...
				recalculationRestFrequencyDayOfWeekType:
					((this.loanProductsTemplate.interestRecalculationData
						...
						!this.loanProductsTemplate.interestRecalculationData
							.recalculationRestFrequencyOnDay) ||
						this.loanProductsTemplate.interestRecalculationData
							.recalculationRestFrequencyType.id === 3) &&
					this.loanProductsTemplate.interestRecalculationData
						.recalculationRestFrequencyWeekday &&
					this.loanProductsTemplate.interestRecalculationData
						.recalculationRestFrequencyWeekday.id,
		...
		if (this.loanProductsTemplate.holdGuaranteeFunds) {
			this.loanProductSettingsForm.patchValue({
				mandatoryGuarantee:
					this.loanProductsTemplate.productGuaranteeData.mandatoryGuarantee,
				minimumGuaranteeFromOwnFunds:
					this.loanProductsTemplate.productGuaranteeData
						.minimumGuaranteeFromOwnFunds,
		...
		if (this.loanProductsTemplate.allowAttributeOverrides) {
			this.loanProductSettingsForm.patchValue({
				...
				allowAttributeOverrides: {
					amortizationType:
						this.loanProductsTemplate.allowAttributeOverrides.amortizationType,
					interestType:
						this.loanProductsTemplate.allowAttributeOverrides.interestType,
					transactionProcessingStrategyCode:
						this.loanProductsTemplate.allowAttributeOverrides
							.transactionProcessingStrategyCode,
					interestCalculationPeriodType:
					...
					inArrearsTolerance:
						this.loanProductsTemplate.allowAttributeOverrides
							.inArrearsTolerance,
					repaymentEvery:
						this.loanProductsTemplate.allowAttributeOverrides.repaymentEvery,
	...
	setConditionalControls() {
		const allowAttributeOverrides = this.loanProductSettingsForm.get(
			"allowAttributeOverrides",
		);
		...
		this.loanProductSettingsForm
			...
			.valueChanges.subscribe((isInterestRecalculationEnabled: any) => {
				if (isInterestRecalculationEnabled) {
					...
					if (
						this.loanProductSettingsForm.value.isInterestRecalculationEnabled
					) {
					...
					this.loanProductSettingsForm
						...
						.valueChanges.subscribe(
							(interestRecalculationCompoundingMethod: any) => {
								if (interestRecalculationCompoundingMethod !== 0) {
									...
									this.loanProductSettingsForm
										...
										.valueChanges.subscribe(
											(recalculationCompoundingFrequencyType: any) => {
						...
						.valueChanges.subscribe((recalculationRestFrequencyType: any) => {
							...
							if (recalculationRestFrequencyType === 3) {
								...
								this.loanProductSettingsForm
									...
									.valueChanges.subscribe(
										(recalculationRestFrequencyNthDayType: any) => {
		...
		this.loanProductSettingsForm
			...
			.valueChanges.subscribe((loanScheduleType: string) => {
				...
				if (loanScheduleType === LoanProducts.LOAN_SCHEDULE_TYPE_CUMULATIVE) {
					...
					if (
						LoanProducts.isAdvancedPaymentAllocationStrategy(
							this.loanProductSettingsForm.value
								.transactionProcessingStrategyCode,
						)
					) {
						this.loanProductSettingsForm.patchValue({
					...
					this.loanProductSettingsForm.addControl(
						"chargeOffBehaviour",
						new UntypedFormControl(
							this.loanProductsTemplate.chargeOffBehaviour.id,
						),
					);
					this.validateAdvancedPaymentStrategyControls();
				}
	...
	private enableFieldsWhenScheduleTypeIsProgressiveAndInterestRateRecalculationEnabled() {
		...
		const isInterestRecalculationEnabled =
			this.loanProductSettingsForm.get("isInterestRecalculationEnabled")
				.value == true;
		const shouldControlExists =
			isProgressiveLoan && isInterestRecalculationEnabled;
	...
	private setRescheduleStrategies() {
		if (this.advancedTransactionProcessingStrategyDisabled) {
			this.rescheduleStrategyTypeData =
				this.rescheduleStrategyTypeDataBase.filter((o: OptionData) => o.id > 3);
			this.loanProductSettingsForm.patchValue({
			...
			this.rescheduleStrategyTypeData =
				this.rescheduleStrategyTypeDataBase.filter((o: OptionData) => o.id < 4);
			this.loanProductSettingsForm.patchValue({
				rescheduleStrategyMethod: this.rescheduleStrategyTypeData[0].id,
			});
	...
	private validateAdvancedPaymentStrategyControls(): void {
		if (this.isAdvancedTransactionProcessingStrategy) {
			const daysInYearType: any =
				this.loanProductSettingsForm.get("daysInYearType").value;
			this.loanProductSettingsForm.addControl(
