...
export class AdvancedAccountingMappingRuleComponent implements OnInit {
	...
	@Output() formChangeEvent: EventEmitter<AdvancedMappingDTO> =
		new EventEmitter<AdvancedMappingDTO>();

	tableData: any[] = [];

	tableDisplayedColumns: string[] = ["codeValueId", "glAccountId", "actions"];
	...
	add() {
		...
		if (this.formType == "ChargeOffReasonExpense") {
			...
			if (
				this.accountingMappingOptions.length == this.currentFormValues.length
			) {
		...
		dialogRef.afterClosed().subscribe((response: any) => {
			if (response.data) {
				if (
					["ChargeOffReasonExpense", "WriteOffReasonToExpense"].includes(
						this.formType,
					)
				) {
					const addData: AccountingMappingDTO = {
						value: this.getValueData(
							response.data.value.chargeOffReasonCodeValueId,
						),
						glAccount: this.getGlAccountData(
							response.data.value.expenseAccountId,
						),
					};
				...
				if (this.formType == "ChargeOffReasonExpense") {
					this.allowAddAccountingMapping =
						this.tableData.length < this.accountingMappingOptions.length;
				}
			}
		});
	}
	...
	addTableData(addedData: AccountingMappingDTO): void {
		let newData = [...this.tableData, addedData];
	...
	edit(record: AccountingMappingDTO, index: number) {
		const data = {
			...this.getData(this.formType, record),
			pristine: false,
	...
	getGlAccountData(valueId: number): GLAccount | null {
		...
		for (const glAccountData of glAccountDatas) {
			if (glAccountData) {
				const glAccount = glAccountData.find(
					(i: GLAccount) => i.id === valueId,
				);
				if (glAccount) {
					return glAccount;
				}
