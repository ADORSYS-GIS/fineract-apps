...
export class LoanProductAccountingStepComponent implements OnInit, OnChanges {
	...
	ngOnInit() {
		...
		this.incomeAndLiabilityAccountData = this.incomeAccountData.concat(
			this.liabilityAccountData,
		);
		this.assetAndLiabilityAccountData =
			this.loanProductsTemplate.accountingMappingOptions
				.assetAndLiabilityAccountOptions || [];
		this.chargeOffReasonOptions =
		...
		this.buydownFeeClassificationOptions =
			this.loanProductsTemplate.buydownFeeClassificationOptions || [];
		this.writeOffReasonOptions =
			...
			case 4:
				this.loanProductAccountingForm.patchValue({
					receivableInterestAccountId:
						accountingMappings.receivableInterestAccount.id,
					receivableFeeAccountId: accountingMappings.receivableFeeAccount.id,
					receivablePenaltyAccountId:
						accountingMappings.receivablePenaltyAccount.id,
				});
				this.loanProductAccountingForm.patchValue({
					enableAccrualActivityPosting:
						this.loanProductsTemplate.enableAccrualActivityPosting,
				});
					...
					if (this.deferredIncomeRecognition.buyDownFee?.enableBuyDownFee) {
						this.loanProductAccountingForm.patchValue({
							...
							incomeFromBuyDownAccountId:
								accountingMappings.incomeFromBuyDownAccount.id,
						});
						if (this.deferredIncomeRecognition.buyDownFee?.merchantBuyDownFee) {
							this.loanProductAccountingForm.patchValue({
								buyDownExpenseAccountId:
									accountingMappings.buyDownExpenseAccount?.id,
							});
						}
					}
				}
			...
			case 2:
				this.loanProductAccountingForm.patchValue({
					...
					incomeFromRecoveryAccountId:
						accountingMappings.incomeFromRecoveryAccount.id,
					writeOffAccountId: accountingMappings.writeOffAccount.id,
					goodwillCreditAccountId:
						accountingMappings.goodwillCreditAccount?.id || null,
					overpaymentLiabilityAccountId:
						accountingMappings.overpaymentLiabilityAccount.id,
					chargeOffFraudExpenseAccountId:
						accountingMappings.chargeOffFraudExpenseAccount
					...
					incomeFromChargeOffPenaltyAccountId:
						accountingMappings.incomeFromChargeOffPenaltyAccount
							? accountingMappings.incomeFromChargeOffPenaltyAccount.id
							: "",
					incomeFromChargeOffFeesAccountId:
						accountingMappings.incomeFromChargeOffFeesAccount
				...
				this.loanProductAccountingForm.setControl(
					...
					this.formBuilder.array(
						(this.loanProductsTemplate.feeToIncomeAccountMappings || []).map(
							(feesIncome: any) => ({
				...
				this.loanProductAccountingForm.setControl(
					...
					this.formBuilder.array(
						(
							this.loanProductsTemplate
								.chargeOffReasonToExpenseAccountMappings || []
						).map((m: ChargeOffReasonToExpenseAccountMapping) => ({
				...
				this.loanProductAccountingForm.setControl(
					...
					this.formBuilder.array(
						(
							this.loanProductsTemplate
								.buydownFeeClassificationToIncomeAccountMappings || []
						).map((m: ClassificationToIncomeAccountMapping) => ({
		...
		this.loanProductAccountingForm
			...
			.valueChanges.subscribe((accountingRule: any) => {
				if (accountingRule >= 2 && accountingRule <= 4) {
					...
					this.loanProductAccountingForm.removeControl(
						"loanPortfolioAccountId",
					);
					this.loanProductAccountingForm.removeControl(
						"transfersInSuspenseAccountId",
		...
		const formfields: FormfieldBase[] = [
			...
			new SelectBase({
				...
				options: {
					label: "name",
					value: "id",
					data: this.incomeAndLiabilityAccountData,
				},
				required: true,
				order: 2,
			}),
	...
	formChangeEvent(accountingData: AdvancedMappingDTO) {
		if (accountingData.formType === "BuydownFeeClassificationToIncome") {
			...
			accountingData.formType === "CapitalizedIncomeClassificationToIncome"
		) {
			this.loanProductAccountingForm.setControl(
				"capitalizedIncomeClassificationToIncomeAccountMappings",
				this.formBuilder.array(
