...
export class LoanProductDetailsStepComponent implements OnInit {
	...
	ngOnInit() {
		...
		this.loanProductDetailsForm.patchValue({
			...
			startDate:
				this.loanProductsTemplate.startDate &&
				new Date(this.loanProductsTemplate.startDate),
			closeDate:
				this.loanProductsTemplate.closeDate &&
				new Date(this.loanProductsTemplate.closeDate),
