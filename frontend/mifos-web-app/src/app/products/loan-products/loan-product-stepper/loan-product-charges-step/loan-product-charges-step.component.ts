...
export class LoanProductChargesStepComponent implements OnInit {
	...
	ngOnInit() {
		...
		this.overdueChargeData = this.loanProductsTemplate.penaltyOptions
			? this.loanProductsTemplate.penaltyOptions.filter(
					(penalty: any) =>
						penalty.chargeTimeType.code === "chargeTimeType.overdueInstallment",
				)
			: [];
		...
		this.multiDisburseLoan.valueChanges.subscribe(
			() => (this.chargesDataSource = []),
		);
	}
	...
	deleteCharge(charge: any) {
		const deleteChargeDialogRef = this.dialog.open(DeleteDialogComponent, {
			data: {
				deleteContext:
					this.translateService.instant("labels.inputs.Charge") +
					" " +
					charge.name,
			},
		});
