...
export class EditLoanProductComponent implements OnInit {
	...
	constructor(
		...
		this.route.data.subscribe(
			(data: { loanProductAndTemplate: any; configurations: any }) => {
				this.loanProductAndTemplate = data.loanProductAndTemplate;
				const assetAccountData =
					this.loanProductAndTemplate.accountingMappingOptions
						.assetAccountOptions || [];
				...
				this.itemsByDefault = loanProducts.setItemsByDefault(
					data.configurations,
				);
	...
	ngOnInit() {
		...
		this.advancePaymentStrategy(
			this.loanProductAndTemplate.transactionProcessingStrategyCode,
		);
		...
		if (this.isAdvancedPaymentStrategy) {
			...
			this.supportedInterestRefundTypes =
				this.loanProductAndTemplate.supportedInterestRefundTypes;
			if (this.loanProductAndTemplate.enableIncomeCapitalization) {
				this.deferredIncomeRecognition.capitalizedIncome = {
					...
					capitalizedIncomeStrategy:
						this.loanProductAndTemplate.capitalizedIncomeStrategy.id,
					capitalizedIncomeType:
						this.loanProductAndTemplate.capitalizedIncomeType.id,
				};
			} else {
				this.deferredIncomeRecognition.capitalizedIncome = {
			...
			if (this.loanProductAndTemplate.enableBuyDownFee) {
				this.deferredIncomeRecognition.buyDownFee = {
					...
					buyDownFeeCalculationType:
						this.loanProductAndTemplate.buyDownFeeCalculationType.id,
					buyDownFeeStrategy: this.loanProductAndTemplate.buyDownFeeStrategy.id,
					buyDownFeeIncomeType:
						this.loanProductAndTemplate.buyDownFeeIncomeType.id,
	...
	setViewChildForm(viewChildForm: UntypedFormGroup): void {
		...
		const capitalizedIncome: CapitalizedIncome =
			formValues.enableIncomeCapitalization
				? {
						enableIncomeCapitalization: true,
						capitalizedIncomeCalculationType:
							formValues.capitalizedIncomeCalculationType,
	...
	advancePaymentStrategy(value: string): void {
		this.isAdvancedPaymentStrategy =
			LoanProducts.isAdvancedPaymentAllocationStrategy(value);
	}
	...
	buildAdvancedPaymentAllocation(): void {
		...
		this.advancedCreditAllocations =
			this.advancedPaymentStrategy.buildAdvancedCreditAllocationList(
				this.loanProductAndTemplate,
			);
	}
	...
	submit() {
		const loanProduct = this.loanProducts.buildPayload(
			this.loanProduct,
			this.itemsByDefault,
		);
		...
		this.productsService
			...
			.subscribe((response: any) => {
				this.router.navigate(["../../", response.resourceId], {
					relativeTo: this.route,
				});
			});
	}

	mapStringEnumOptionToIdList(
		incomingValues: StringEnumOptionData[],
	): string[] {
