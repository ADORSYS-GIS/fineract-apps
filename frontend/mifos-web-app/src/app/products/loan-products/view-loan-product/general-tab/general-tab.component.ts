...
export class GeneralTabComponent implements OnInit {
	...
	exportDefinition(): void {
		...
		link.setAttribute(
			...
			"data:text/plain;charset=utf-u," +
				encodeURIComponent(JSON.stringify(product, null, 2)),
		);
	...
	copyProduct(): void {
		...
		const data = {
			...
			layout: {
				addButtonText: this.translateService.instant("labels.buttons.Create"),
			},
			formfields: formfields,
		};
		const createProductDialogRef = this.dialog.open(FormDialogComponent, {
			data,
		});
		createProductDialogRef.afterClosed().subscribe((productResponse: any) => {
			if (productResponse.data) {
				...
				productCopy["currencyCode"] = productCopy["currency"]
					...
					: null;
				productCopy["interestRatePerPeriod"] =
					productCopy["annualInterestRate"];
