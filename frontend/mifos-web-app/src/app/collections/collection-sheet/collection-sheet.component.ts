...
export class CollectionSheetComponent implements OnInit {
	...
	buildDependencies() {
		this.collectionSheetForm
			.get("officeId")
			.valueChanges.subscribe((officeId: any) => {
	...
	previewCollectionSheet() {
		...
		const meetingDate = this.dateUtils.formatDate(
			this.collectionSheetForm.value.meetingDate,
			dateFormat,
		);
		...
		this.centerService
			...
			.subscribe((response: CollectionSheetData[]) => {
				if (response.length > 0) {
					...
					const payload = {
						calendarId:
							this.meetingFallCenters[0].collectionMeetingCalendar
								.calendarInstanceId,
