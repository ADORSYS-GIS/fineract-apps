...
export class IndividualCollectionSheetComponent implements OnInit {
	...
	getLoanTotalDueAmount(loan: any) {
		...
		return (
			Math.ceil((Number(principalInterestDue) + Number(chargesDue)) * 100) / 100
		);
	}
	...
	showAndStorePaymentDetails(type: any, selectedData: any, index: number) {
		const formfields: FormfieldBase[] = [
			new SelectBase({
				...
				options: {
					label: "name",
					value: "id",
					data: this.collectionSheetData.paymentTypeOptions,
				},
				required: false,
			}),
			new InputBase({
		...
		const showPaymentDetailsDialogRef = this.dialog.open(FormDialogComponent, {
			data,
		});
			...
			if (response.data) {
				if (type === "loans") {
					...
					if (response.data.value.paymentTypeId !== "") {
						loanTransaction["paymentTypeId"] =
							response.data.value.paymentTypeId;
						loanTransaction["accountNumber"] =
							response.data.value.accountNumber;
						loanTransaction["checkNumber"] = response.data.value.checkNumber;
						loanTransaction["routingCode"] = response.data.value.routingCode;
						loanTransaction["receiptNumber"] =
							response.data.value.receiptNumber;
