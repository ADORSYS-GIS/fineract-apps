...
export class GroupAttendanceComponent implements OnInit {
	...
	ngOnInit() {
		...
		this.meetingDates =
			this.groupData.collectionMeetingCalendar.recurringDates.filter(
	...
	getAttendanceOptions() {
		this.groupsService
			.getMeetingsTemplate(
				this.groupData.id,
				this.groupData.collectionMeetingCalendar.id,
			)
	...
	editAttendance(member: any) {
		const formfields: FormfieldBase[] = [
			new SelectBase({
				...
				options: {
					label: "value",
					value: "id",
					data: this.attendanceTypeOptions,
				},
				required: false,
			}),
		];
		...
		const memberAttendanceDialogRef = this.dialog.open(FormDialogComponent, {
			data,
		});
		memberAttendanceDialogRef.afterClosed().subscribe((response: any) => {
			if (response.data) {
				...
				this.dataSource.splice(
					this.dataSource.indexOf(member),
					1,
					updatedMemeber,
				);
