...
export class CreateGroupComponent implements OnInit, AfterViewInit {
	...
	submit() {
		...
		if (groupFormData.submittedOnDate instanceof Date) {
			groupFormData.submittedOnDate = this.dateUtils.formatDate(
				submittedOnDate,
				dateFormat,
			);
		}
		if (groupFormData.activationDate instanceof Date) {
			groupFormData.activationDate = this.dateUtils.formatDate(
				activationDate,
				dateFormat,
			);
		}
		...
		data.clientMembers = [];
		this.clientMembers.forEach((client: any) =>
			data.clientMembers.push(client.id),
		);
