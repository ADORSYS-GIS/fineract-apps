...
export class EditGroupComponent implements OnInit {
	...
	constructor(
		...
		this.route.data.subscribe(
			(data: { groupAndTemplateData: any; groupViewData: any }) => {
	...
	buildDependencies() {
		if (this.groupData.active) {
			...
			this.editGroupForm
				.get("activationDate")
				.patchValue(
					this.groupData.activationDate &&
						new Date(this.groupData.activationDate),
				);
		} else {
	...
	submit() {
		...
		if (editGroupFormData.submittedOnDate instanceof Date) {
			editGroupFormData.submittedOnDate = this.dateUtils.formatDate(
				submittedOnDate,
				dateFormat,
			);
		}
		if (editGroupFormData.activationDate instanceof Date) {
			editGroupFormData.activationDate = this.dateUtils.formatDate(
				activationDate,
				dateFormat,
			);
		}
		...
		this.groupService
			.updateGroup(data, this.groupData.id)
