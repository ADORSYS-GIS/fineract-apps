				...
				switch (timescale) {
					case "Day":
						...
						forkJoin([clientsByDay, loansByDay]).subscribe((data: any[]) => {
							...
							const clientCounts = this.getCounts(
								data[0],
								dayLabels,
								timescale,
								"client",
							);
							const loanCounts = this.getCounts(
								data[1],
								dayLabels,
								timescale,
								"loan",
					...
					case "Month":
						...
						forkJoin([clientsByMonth, loansByMonth]).subscribe(
							(data: any[]) => {
			...
			case "Day":
				labels.forEach((label: any) => {
					const day = response.find((entry: any) => {
						const transformedDate = this.dateUtils.formatDate(
							entry.days,
							"d/M",
						);
