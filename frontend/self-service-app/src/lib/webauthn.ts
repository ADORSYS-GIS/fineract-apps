	...
	try {
		...
		if (error instanceof Error) {
			if (error.name === "NotAllowedError") {
				throw new Error(
					"User cancelled the registration or the operation timed out",
				);
			}
...
export async function authenticateWithWebAuthn(
	...
	try {
		...
		if (error instanceof Error) {
			if (error.name === "NotAllowedError") {
				throw new Error(
					"User cancelled the authentication or the operation timed out",
				);
			}
			if (error.name === "InvalidStateError") {
...
export async function requestStepUpAuth(
	...
	try {
		...
		const optionsResponse = await fetch(
			`${apiBaseUrl}/webauthn/authenticate/options`,
			{
				method: "POST",
				headers: {
		...
		const verifyResponse = await fetch(
			`${apiBaseUrl}/webauthn/authenticate/verify`,
			{
				method: "POST",
				headers: {
					"Content-Type": "application/json",
