...
const defaultLimits: Record<number, Omit<TransactionLimits, "tier" | "paymentMethods">> = {
  1: {
    deposit: {
      daily: 50000,
...
export function useLimits() {
  ...
  const availablePaymentMethods = (query.data?.paymentMethods || defaultPaymentMethods).filter(
    (method) => method.requiresKycTier <= kycTier
  );
