...
import { ArrowDownCircle, ArrowUpCircle, Filter, Search, Loader2, AlertCircle } from "lucide-react";
...
function TransactionsPage() {
  ...
  const { data: transactions = [], isLoading: isLoadingTransactions, error } = useTransactions(account?.id);

  const currency = account?.currency?.code || "XAF";
  const isLoading = isLoadingCustomer || isLoadingAccount || isLoadingTransactions;
  ...
  const getPaymentMethod = (tx: typeof transactions[0]) => {
    ...
    const paymentType = tx.paymentDetail?.paymentType?.name?.toLowerCase() || "";
  ...
  const filteredTransactions = transactions.filter((tx) => {
    ...
    if (searchTerm && !reference.toLowerCase().includes(searchTerm.toLowerCase())) {
      return false;
    }
    return true;
  });
                    ...
                    <div>
                      ...
                      <p className="text-sm text-gray-500">
                        {paymentMethod.includes("_")
                          ? t(`paymentMethods.${paymentMethod}`, { defaultValue: paymentMethod })
                          : paymentMethod}
                      </p>
