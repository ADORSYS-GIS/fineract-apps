...
import { Copy, CheckCircle, Loader2, AlertCircle, CreditCard, Building2, Calendar, TrendingUp } from "lucide-react";
import { useState } from "react";
import { useCustomer, usePrimarySavingsAccount, useTransactions } from "../hooks";
...
function AccountPage() {
  ...
  const getStatusBadge = () => {
    ...
    return (
      <span className={`px-2 py-1 rounded-full text-xs font-medium ${statusColors[status.code] || "bg-gray-100 text-gray-700"}`}>
        {status.value}
      </span>
    );
  };
  ...
  return (
    <div className="space-y-6">
      {/* Header */}
      <div>
        <h1 className="text-2xl font-bold text-gray-900">{t("nav.account", { defaultValue: "Account Details" })}</h1>
        <p className="text-gray-500 mt-1">View your savings account information</p>
      </div>
      ...
      <div className="card bg-gradient-to-br from-blue-600 to-blue-700 text-white">
        ...
        {/* Account Number */}
        <div className="mb-6">
          <p className="text-blue-200 text-xs uppercase tracking-wide">Account Number</p>
          <div className="flex items-center gap-3 mt-1">
            <p className="text-2xl font-mono tracking-wider">{account.accountNo}</p>
            <button
      ...
      {/* Account Holder */}
      <div className="card">
        <h3 className="text-sm font-medium text-gray-500 mb-4">Account Holder</h3>
        <div className="flex items-center gap-4">
          ...
          <div>
            <p className="font-medium text-gray-900">{customer?.displayName || "Customer"}</p>
            <p className="text-sm text-gray-500">
              {customer?.mobileNo || customer?.emailAddress || ""}
            </p>
          </div>
        </div>
      </div>
      ...
      {/* Account Summary */}
      <div className="card">
        <h3 className="text-sm font-medium text-gray-500 mb-4">Account Summary</h3>
        <div className="grid grid-cols-2 gap-4">
          <div className="p-4 bg-green-50 rounded-lg">
            ...
            <p className="text-xs text-gray-500 mt-1">{summary.depositCount} transactions</p>
          </div>
          ...
          <div className="p-4 bg-red-50 rounded-lg">
            ...
            <p className="text-xs text-gray-500 mt-1">{summary.withdrawalCount} transactions</p>
          </div>
        </div>
      </div>
      ...
      <div className="card">
        ...
        <div className="space-y-3">
          <div className="flex items-center justify-between py-2 border-b border-gray-100">
            ...
            <span className="font-medium text-gray-900">{account.productName}</span>
          </div>
          <div className="flex items-center justify-between py-2 border-b border-gray-100">
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <a
          ...
          className="card flex flex-col items-center justify-center py-6 hover:border-blue-300 hover:shadow-md transition-all"
        >
          <span className="text-blue-600 font-medium">{t("nav.transactions")}</span>
        </a>
      </div>
    </div>
  );
