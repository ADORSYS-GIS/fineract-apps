		...
		>
			...
			<div className="border-t pt-4">
				...
				<ul className="space-y-1">
					{tierRequirements[tier as keyof typeof tierRequirements]?.map(
						(req) => (
							<li
								key={req}
		...
		<div className="space-y-4">
			<div>
				<div className="flex justify-between items-center mb-1">
					...
					<span className="text-sm text-gray-700">
						{formatCurrency(dailyUsed, currency)} /{" "}
						{formatCurrency(limits.daily, currency)} {currency}
					</span>
				</div>
				...
				<p className="text-xs text-gray-400 mt-1">
					{t("transactions.limits.remaining")}:{" "}
					{formatCurrency(limits.remaining.daily, currency)} {currency}
				</p>
			</div>
			...
			<div>
				<div className="flex justify-between items-center mb-1">
					<span className="text-sm text-gray-500">Monthly</span>
					<span className="text-sm text-gray-700">
						{formatCurrency(monthlyUsed, currency)} /{" "}
						{formatCurrency(limits.monthly, currency)} {currency}
					</span>
				</div>
				<div className="w-full bg-gray-200 rounded-full h-2">
				...
				<p className="text-xs text-gray-400 mt-1">
					{t("transactions.limits.remaining")}:{" "}
					{formatCurrency(limits.remaining.monthly, currency)} {currency}
				</p>
			</div>
		</div>
	);
