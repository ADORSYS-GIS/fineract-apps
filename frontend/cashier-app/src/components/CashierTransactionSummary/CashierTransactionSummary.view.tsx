...
const LoadingSkeleton = () => (
	<div className="grid grid-cols-1 md:grid-cols-2 gap-4 p-4 animate-pulse">
		<Card className="p-4">
			...
			<Card className="p-4">
				...
				<div className="space-y-2">
					...
					<div className="flex justify-between">
						...
						<p className="font-semibold text-gray-900">
							{formatCurrency(
								cashierData?.cashierTransactions?.pageItems?.reduce(
									(acc, item) => acc + (item.txnAmount ?? 0),
									0,
								) ?? 0,
							)}
						</p>
								...
								{cashierData?.cashierTransactions?.pageItems?.map((item) => (
									...
									>
										<td className="px-6 py-4">
											{item.txnDate
												? format(new Date(item.txnDate), "PPP")
												: ""}
										</td>
										<td className="px-6 py-4">
