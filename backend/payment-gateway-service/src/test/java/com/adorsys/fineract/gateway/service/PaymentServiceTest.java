...
class PaymentServiceTest {
    ...
    @Mock private CallbackHandlerDelegate callbackDelegate;
    @Mock private TransactionReservationService reservationService;
    ...
    // =========================================================================
    // Callback handler delegation tests
    // Callback logic is now in CallbackHandlerDelegate.
    // These tests verify PaymentService correctly delegates and handles reversal.
    // =========================================================================
    ...
    class MtnCollectionCallbackTests {
        ...
        @Test
        @DisplayName("should delegate to callbackDelegate")
        void handleMtnCollectionCallback_delegatesToDelegate() {
    ...
    class MtnDisbursementCallbackTests {
        ...
        @Test
        @DisplayName("should not call reversal when delegate returns no reversal needed")
        void handleMtnDisbursementCallback_success_noReversal() {
            MtnCallbackRequest callback = MtnCallbackRequest.builder()
            ...
            verify(reversalService, never()).reverseWithdrawal(any());
        }
    ...
    class OrangeCallbackTests {
        ...
        void handleOrangeCallback_withdrawalFailed_callsReversal() {
            ...
            when(callbackDelegate.processOrangeCallback(callback))
                    .thenReturn(CallbackHandlerDelegate.CallbackResult.needsReversal(txn));
    ...
    class CinetPayCallbackTests {
        ...
        @Test
        @DisplayName("should delegate to callbackDelegate")
        void handleCinetPayCallback_delegatesToDelegate() {
            CinetPayCallbackRequest callback = CinetPayCallbackRequest.builder()
                    .transactionId("txn-cp1")
                    .resultCode("00")
                    .build();

            when(callbackDelegate.processCinetPayCallback(callback))
                    .thenReturn(CallbackHandlerDelegate.CallbackResult.noReversal());

            paymentService.handleCinetPayCallback(callback);

            verify(callbackDelegate).processCinetPayCallback(callback);
            verify(reversalService, never()).reverseWithdrawal(any());
        }
    }
